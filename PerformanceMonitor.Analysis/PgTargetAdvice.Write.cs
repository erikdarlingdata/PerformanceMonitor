...
public static partial class PgTargetAdvice
{
    ...
    private static AdviceBlock ComposeCheckpointPressure(IReadOnlyDictionary<string, Fact> facts)
    {
        ...
        /* #3955: PostgreSQL counts a shutdown checkpoint as requested and keeps the count across the restart, so the
           collector left the restart-spanning intervals out of the requested count rather than read them as WAL. */
        ...
        if (restarts > 0)
            sb.Append("PostgreSQL restarted ").Append(KnobNum(restarts)).Append(" time(s) inside the window; a shutdown checkpoint " +
                      "counts as requested and the count survives the restart, so the collection interval(s) spanning a restart " +
                      "are left out of the requested count rather than read as WAL pressure. ");
