...
public sealed class McpToolsListBudgetTests
{
    ...
    /// </list>
    /// </summary>
    private const int TotalCeilingBytes = 147_053;
    ...
    public void EveryServedDescription_AndEveryParameterDescription_MatchesItsCeiling()
    {
        ...
        if (problems.Count > 0)
        {
            var path = Path.Combine(Path.GetTempPath(), "lite-" + BudgetFile);
    ...
    private static Dictionary<string, int> ReadBudget()
    {
        ...
        foreach (var raw in File.ReadAllLines(RepoPath("Lite.Tests", BudgetFile)))
        {
            var line = raw.Trim();
    ...
    private static string Render(Measurement measured)
    {
        ...
        sb.AppendLine("# #3898 Phase 0: Lite's tools/list budget. Ceilings only go down; see McpToolsListBudgetTests.");
