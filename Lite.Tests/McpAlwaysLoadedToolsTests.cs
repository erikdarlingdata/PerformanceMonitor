...
/// <summary>
/// #3898 D10: exactly the entry tools Lite has — <c>list_servers</c>, <c>analyze_server</c> and
/// <c>get_tool_guide</c> — carry <c>_meta["anthropic/alwaysLoad"] = true</c> in <c>tools/list</c>, so
/// Claude Code keeps them loaded when it defers the rest (Claude Code MCP docs, "Exempt a server from
/// deferral": "An MCP server can also mark individual tools as always-loaded by including
/// <c>"anthropic/alwaysLoad": true</c> in the tool's <c>_meta</c> object"). Lite has no
/// <c>get_fleet_overview</c> (central-store-only, Darling only). Built through the same
/// <c>WithGeminiCompatibleTools</c> path the host registers every tool with — no running server
/// required. Darling twin: <see cref="Darling.Tests.McpAlwaysLoadedToolsTests"/>.
/// </summary>
public class McpAlwaysLoadedToolsTests
{
    ...
    {
        var toolTypes = typeof(PerformanceMonitorLite.Mcp.McpWaitTools).Assembly
            .GetTypes()
        ...
        var serviceParamTypes = toolTypes
            ...
            .Where(t => McpServedSchema.IsServiceParameter(t) && t != typeof(PerformanceMonitor.Common.McpToolGuideCatalog))
            .Distinct();
    ...
    [Fact]
    public void ExactlyTheEntryTools_CarryAlwaysLoadMeta()
    {
