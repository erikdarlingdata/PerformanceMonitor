...
/// <summary>
/// #3898 D3 head pins for <c>analyze_query_plan</c>'s Lite twin (wave E, lane e1, D9 finish pass). Darling's
/// twin is <c>Darling.Tests/McpToolGuideHeadsAnalyzeQueryPlanTests</c>, which also holds the shared-block
/// rationale: the head is the same guardrail block the plan-tool truth pins share with <c>analyze_plan_xml</c>
/// (#4115) and <c>analyze_procedure_plan</c> (#4117).
/// </summary>
public sealed class McpToolGuideHeadsAnalyzeQueryPlanTests
{
    ...
    /// <summary>D9: nothing dropped. Every sentence of the pre-#3898 description, including Lite's own
    /// product-specific first sentence, survives verbatim in the tail get_tool_guide serves.</summary>
    [Fact]
    public void Tail_KeepsEveryOriginalSentence_IncludingLitesOwnFirstOne()
    {
        var served = McpToolGuideTests.Served(Tool);
        Assert.NotNull(served.Tail);
        Assert.StartsWith("Analyzes an execution plan from the plan cache by query_hash.", served.Tail!, StringComparison.Ordinal);
