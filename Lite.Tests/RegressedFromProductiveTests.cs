...
public sealed class RegressedFromProductiveTests
{
    ...
    /// </summary>
    private static CollectorHealthRow Regressed() => new()
    {
    ...
    /// </summary>
    private static CollectorHealthRow NeverProduced() => new()
    {
        CollectorName = "pg_statement_stats",
        TotalRuns = 10_080,
        SuccessCount = 0,
    ...
    /// <summary>
    /// Lite's half of the #2804 / #3240 / #3819 invariant: this SKU's banding read must select the two
    /// instants the regression floor reads, or the grid calls a regressed collector HEALTHY while the
    /// service's twin calls it WARNING. Darling.Tests holds the other four reads.
    ///
    /// <para><b>And it is not inert here the way #3240's count is.</b> Lite's SQL Server collectors never
    /// write EXTENSION_MISSING or SESSION_MISSING, but they do write PERMISSIONS — so the streak this
    /// detects on this SKU is a grant that was held and has been revoked since, which is the same
    /// regression with a narrower population.</para>
    /// </summary>
    [Fact]
    public void TheBandingRead_SelectsBothRegressionInstants()
    {
        ...
        Assert.Contains(CollectorRuntimePrecondition.NamedSkipStatusSqlList, sql, StringComparison.Ordinal);

        /* The NULL arm, which decides what an unwritten status does: it counts as NON-skip, so a status
           this build never wrote cannot manufacture a streak. */
        Assert.Contains("MAX(CASE WHEN status IS NULL", sql, StringComparison.Ordinal);
    }

    /// <summary>
    /// The WIRING pin, because no behavioural test reaches it: the fields are emitted by an anonymous
    /// object inside an MCP tool method that needs a live store to invoke, and BOTH SKUs' tools have to
    /// carry them or an operator moving between them is told a different story about the same state.
    /// </summary>
    [Theory]
    [InlineData("Lite/Mcp/McpHealthTools.cs")]
    [InlineData("Darling/PerformanceMonitor.Darling.Service/Mcp/DarlingMcpDataTools.cs")]
    public void BothSkusTools_EmitTheRegressionFields(string relative)
    {
