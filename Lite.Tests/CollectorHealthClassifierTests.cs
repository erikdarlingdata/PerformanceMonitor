...
public sealed class CollectorHealthClassifierTests
{
    ...
    /// <summary>
    /// #3240, Lite's half of the five-read invariant the four Darling banding reads are pinned for in
    /// <c>Darling.Tests/CollectorHealthClassifierTests</c>: the read must SELECT the extension-missing
    /// count, because an unselected count defaults to 0 and COMPILES. On this SKU the count is always 0
    /// (SQL Server collectors never write the status), but the two health reads are ordinal twins and
    /// the shared classifier takes the input — a Lite read left behind would be the #2779/#2784 shape
    /// waiting for the first shared consumer.
    /// </summary>
    [Fact]
    public void TheCollectionHealthRead_SelectsTheExtensionMissingCount_SoTheSharedClassifierIsFullyFed()
    {
        var sql = LocalDataService.CollectionHealthSql;
