...
| MCP server (LLM integration) | Built-in (87 tools) | On request (148 tools) | Built into Dashboard (66 tools) |
...
**Lite** exposes 87 tools; **Darling** exposes 148 (the analysis + data-read surface plus its write tools) on request; the deprecated **Dashboard** exposes 66 (see [deprecated/Dashboard/README.md](deprecated/Dashboard/README.md)). Core tools are shared.
...
- **Only one device-code sign-in can be in flight at a time**, and a second is refused with a message rather than queued. The driver's callback carries no connection id, so two at once cannot be told apart — the first sign-in's code would be displayed on the second one's window. Finish or close the one on screen and try again.
