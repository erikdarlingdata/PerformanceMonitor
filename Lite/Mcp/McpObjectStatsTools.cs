...
public sealed class McpObjectStatsTools
{
    ...
    [McpServerTool(Name = "get_index_usage"), Description("Per-index usage (seeks, scans, lookups, updates) from the latest daily snapshot, classed Unused, Write-only, or Active. Unused/write-only sort first as drop candidates: on a server with many, results can be one database's unused indexes, hiding Active ones elsewhere. Counters reset at the last restart or index rebuild. last_user_access is UTC, de-skewed from local clock: compare directly against get_collection_log and list_servers. <<GUIDE>> Gets per-index usage (seeks, scans, lookups, updates) from the latest daily snapshot, classifying each index as Unused, Write-only, or Active. Unused and write-only indexes are listed first - these are drop candidates. Counters are cumulative since the last instance restart. last_user_access is UTC - the underlying sys.dm_db_index_usage_stats columns are in the monitored server's local clock and this read de-skews them - so it compares directly against get_collection_log and list_servers. Classification: Unused is zero seeks, scans and lookups AND zero updates; Write-only is zero of the first three but at least one update; everything else is Active. sys.dm_db_index_usage_stats also clears on an index rebuild and on a database detach/reattach, not only on an instance restart, so a heavily-used index that was just rebuilt can read as Unused until it accrues new activity - check the index's maintenance history before treating an Unused row as a drop candidate. This read has no database_name filter and no truncation signal: it always returns every database's indexes on the server together, capped at 200 rows, unused and write-only first. On a server with 200 or more unused or write-only indexes, the answer can be entirely one or two databases' unused indexes with no Active row at all and nothing in the response saying more exist - a short, all-Unused answer here is not proof the server has few Active indexes. A zero-row result is always not_collected (if the engine's collection state says so) or unavailable; this tool never returns status empty.")]
