...
public sealed class McpHealthTools
{
        ...
        [Description("Return every field for every collector. Default compacts a HEALTHY collector with nothing to report; failing, stale, stopped, erroring, denied or regressed collectors always keep every field.")] bool full_detail = false)
    {
        ...
        try
        {
            ...
            var result = rows.Select(r => !full_detail && IsCollectionHealthCompactEligible(r)
            ...
            return JsonSerializer.Serialize(CollectionCaveatLedger.Shared.Attach(new
            {
                ...
                collector_detail_note = full_detail
                    ...
                    : $"{compactCount} of {rows.Count} collector(s) below are HEALTHY with nothing to report (no errors, denials, session/extension-missing runs or abandoned cycles this window) and are compacted to collector/status/compact/total_runs/rows_stored/avg_duration_ms/last_success. Any collector that is failing, stale, stopped, erroring, denied, regressed, or whose zero output needs a look always keeps every field. Pass full_detail=true for every field on every collector.",
                collectors = result
    ...
    public static async Task<string> GetCollectionLog(
        ...
        [Description("Maximum rows to return, applied after the filters. Default 200 for one server; the fleet-wide form (server_name omitted or \"*\") defaults lower — see tool guide.")] int? limit = null,
