...
internal static class McpInstructions
{
    public const string Text = """
        ...
        | `get_query_trend` | Time-series for a specific query by query_hash. Carries the same `source` / `effective_start` / `truncated` / `bucket` block as the duration trends (always `raw`, `per-collection` on Lite) | `query_hash` (required), `database_name` (required), `server_name`, `hours_back`, `as_of` |
        ...
        | `get_procedure_duration_trend` | The same series over procedure_stats. NOT a duplicate: query_stats smears a procedure's work across the statements inside it, this charges the whole call to the procedure. Read both to tell an ad-hoc regression from a procedure regression. Same `source` / `effective_start` / `truncated` / `bucket` block as `get_query_duration_trend`, and the same rating rule (the STORED interval; a restart's 0 is unrated) | `server_name`, `hours_back`, `as_of` |
        ...
        A tool that caught an exception answers with the `{"status":"error","message":...}` envelope described under "Reading an empty result" — the message carries the cause, and `hints.operation` names the tool. The messages below are refusals and misses, not failures:
