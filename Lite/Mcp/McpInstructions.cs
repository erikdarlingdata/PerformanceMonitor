...
internal static class McpInstructions
{
    public const string Text = """
        ...
        | `get_query_duration_trend` | Overall query elapsed-ms/sec + executions/sec over time, from the PLAN CACHE. `execution_count` and `executions_per_second` are the same quantity; the first is truncated to an integer, so read the second on a quiet server. Each point also carries `elapsed_ms_per_second`, the same quantity as `value` with its unit in the name — read that one. The payload says what it served: `source` (always `raw` on Lite — DuckDB keeps every collected row for the collector's `retention_days`, nothing rolls them up), `bucket` (`per-collection`), `effective_start` / `effective_hours_back` (where the series the store actually held begins) and `window_truncated` (true when that head sits later than asked — a server added mid-window, or a retention purge shorter than `hours_back`; the WINDOW floor, spelled `truncated` before #3653 and renamed because that is the paged tools' word for a `limit` biting, a different fact with the opposite remedy). Same field set as Darling's twin, whose `source` can also be `hourly`. An empty result carries the same block and distinguishes a quiet window (`empty`, widen `hours_back`) from a server nothing has ever been collected for (`unavailable`, collection is not running) Each point is a rate over the collection's STORED `sample_interval_seconds`, so a collection whose interval was unknowable (a restart or counter reset, stored as 0) carries null rates, and a collection that recorded no interval is rated over the gap since the PREVIOUS collection, so the window's first such collection carries null rates too (`unrated_points` / `unrated_note`): unknowable, never reported as 0. | `server_name`, `hours_back`, `as_of` |
        | `get_procedure_duration_trend` | The same series over procedure_stats. NOT a duplicate: query_stats smears a procedure's work across the statements inside it, this charges the whole call to the procedure. Read both to tell an ad-hoc regression from a procedure regression. Same `source` / `effective_start` / `window_truncated` / `bucket` block as `get_query_duration_trend`, and the same rating rule (the STORED interval; a restart's 0 is unrated) | `server_name`, `hours_back`, `as_of` |
