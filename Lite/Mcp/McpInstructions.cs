...
internal static class McpInstructions
{
    public const string Text = """
        ...
        | `get_collection_health` | Shows collector health: running, failing, or stale, plus the server's sweep_pressure block: a `verdict` for SUSTAINED demand (a SATURATED body collects at a multiple of its configured cadence with every collector healthy) and a separate `peak_cycle_risk` for a SINGLE sweep (BODY_OVERRUN means one scheduled body cannot fit the budget even when the verdict reads OK, the signature of one infrequent heavy collector; `peak_collector` names it). Per-collector rows carry `avg_duration_ms`, `p95_duration_ms` and `max_duration_ms`: a mean far below the p95 means the collector's runs come in two sizes and the mean describes neither; carries analysis_caveats when a recent analysis pass could not read a fact family | `server_name` |
        | `get_collection_log` | The RAW per-run collection log behind that rollup: one row per collector run with total duration split into time on the monitored server and time on the store, rows collected, status and any error. Reach for it when the rollup reads HEALTHY and collection still looks wrong, or to see what a collector was doing during a specific window. Newest first, or SLOWEST first when `min_duration_ms` is supplied — a duration floor under newest-first ordering cannot reach the tail, so the two are one decision and `order` names which you got. All THREE filters are applied in SQL BEFORE the cap, so `run_count` and `truncated` describe the MATCHING rows. `status` is the FAILURE filter and the one to reach for during an incident — "show me the failures" is what this log exists to answer, and paging the newest-first tail cannot reach a failure that is not recent; an unknown value is REFUSED naming the accepted set, never applied as a filter that matches nothing, and the applied value is echoed as `status_filter` (not `status`, which on a miss is the miss word). `hours_back` is the span you asked for; `oldest_returned_collection_time` and `newest_returned_collection_time` bound the PAGE you got (under the default ordering that is also the reach; under a duration floor the page is a cost-ranked sample and its oldest row says nothing about reach), and the cap can make those differ by orders of magnitude — read them before concluding anything from the rows. An empty result distinguishes THREE states: filters that matched nothing (`empty`, and it says nothing about the window as a whole), a quiet window (`empty`, widen it), and a server that has never collected (`unavailable`, collection is not running) | `server_name`, `hours_back`, `limit`, `as_of`, `collector_name`, `min_duration_ms`, `status` |
        ...
        | `get_daily_summary` | Daily composite health band + wait/query/deadlock/blocking/CPU/memory/alert rollup for one day. A day before the store's `retention_horizon` is `unavailable` with `data_state: purged` — never a band, because its zeros are absences | `server_name`, `summary_date` (yyyy-MM-dd ONLY, refused otherwise; default today) |
        | `get_daily_summary_range` | The SAME rollup across a span of days — one row per collected day, the Performance Calendar's month grid. Use it when the question is WHICH day rather than how one day went: scan the bands, then call `get_daily_summary` for the day that stands out. A day with ANY collection appears even when every signal was quiet, so a day absent from the result is a gap in COLLECTION — inside retention. The payload publishes `retention_horizon` (the oldest day every signal table still holds) and `days_before_horizon`; a returned day before the horizon is `data_state: purged` (no signal table holds it) or `past_horizon` (some still do) and `NoData`, NEVER Healthy, because a purged day's per-signal zeros are absences left by the purge. Purged and past-horizon rows carry no verdict; `no_run_record` (inside retention, no collector run recorded) keeps its band with a caveat. `as_of` anchors the LAST day of the range. An empty result distinguishes a range outside this server's history (`empty`) from a server nothing has ever been collected for (`unavailable`) | `server_name`, `days_back` (default 30, max 366), `as_of` |
        ...
        | Tool | Purpose | Key Parameters |
        |------|---------|----------------|
        | `get_latch_stats` | Latest latch-contention snapshot by class (waits + last-interval delta waits, `waits_per_second` / `wait_ms_per_second` over `interval_seconds` — all null with the deltas on a restart row, the same keys Darling's twin spells); `captured_at` / `age_seconds`; a PAGE — `latches_returned` / `truncated` | `server_name`, `hours_back` (default 24; the span SEARCHED for the newest snapshot), `limit` (default 20), `as_of` |
        | `get_spinlock_stats` | Latest spinlock-contention snapshot (collisions, spins, backoffs; `collisions_per_second` / `spins_per_second` over `interval_seconds` — all null with the deltas on a restart row); `captured_at` / `age_seconds`; a PAGE — `spinlocks_returned` / `truncated` (well over a hundred spinlocks exist; the default page is the hot tail) | `server_name`, `hours_back` (default 24; the span SEARCHED for the newest snapshot), `limit` (default 20), `as_of` |
        ...
        | `get_query_store_regressions` | Queries whose Query Store performance got WORSE: the recent window's averages vs the BASELINE (every capture before it), with duration / CPU / reads regression percents, the execution-count-weighted extra duration (the ranking key), the plan counts on both sides and a severity band. `get_query_store_top` answers what is EXPENSIVE; this answers what CHANGED. Kept only where average CPU regressed > 25%. Zero rows is four states and the read says which — including "no baseline, so no regression is detectable", which is NOT a clean bill of health A percent whose baseline side is 0 (e.g. no logical reads before the window, 50k inside it) has no denominator and is null with the reason under `undefined_percents` — never 0, which would read as no change; the ranking key is an absolute delta and exists for every row, and `severity` is null when the duration percent is. | `server_name`, `hours_back`, `database_name`, `limit`, `as_of` |
        | `get_query_trend` | Time-series for a specific query by query_hash. Carries the same `source` / `effective_start` / `window_truncated` / `bucket` block as the duration trends (always `raw`, `per-collection` on Lite; the window floor is `window_truncated`, spelled `truncated` before #3653) | `query_hash` (required), `database_name` (required), `server_name`, `hours_back`, `as_of` |
        | `get_query_duration_trend` | Overall query elapsed-ms/sec + executions/sec over time, from the PLAN CACHE. `execution_count` and `executions_per_second` are the same quantity; the first is truncated to an integer, so read the second on a quiet server. Each point also carries `elapsed_ms_per_second`, the same quantity as `value` with its unit in the name — read that one — and `peak_elapsed_ms_per_second`, the point's busiest collection. Points are time buckets sized to the window (`bucket` / `bucket_minutes`, `aggregate_note`). The payload says what it served: `source` (always `raw` on Lite — DuckDB keeps every collected row for the collector's `retention_days`, nothing rolls them up), `effective_start` / `effective_hours_back` (where the series the store actually held begins) and `window_truncated` (true when that head sits later than asked — a server added mid-window, or a retention purge shorter than `hours_back`). Same field set as Darling's twin, whose `source` can also be `hourly`. An empty result carries the same block and distinguishes a quiet window (`empty`, widen `hours_back`) from a server nothing has ever been collected for (`unavailable`, collection is not running). Rates are over each collection's STORED `sample_interval_seconds`; a collection whose interval was unknowable (a restart or counter reset, stored as 0, or the window's first collection when none was stored) is left out of its bucket, never counted as 0 (`unrated_collections`), and a bucket holding nothing else carries null rates (`unrated_points` / `unrated_note`). | `server_name`, `hours_back`, `as_of`, `bucket_minutes` |
        | `get_procedure_duration_trend` | The same series over procedure_stats. NOT a duplicate: query_stats smears a procedure's work across the statements inside it, this charges the whole call to the procedure. Read both to tell an ad-hoc regression from a procedure regression. Same buckets, disclosure block and rating rule as `get_query_duration_trend` | `server_name`, `hours_back`, `as_of`, `bucket_minutes` |
        | `get_query_store_duration_trend` | The same series over Query Store, which persists per interval and survives a plan-cache eviction or a restart. Each interval is counted once, at the hour the work RAN. Its `unavailable` names the cause the plan-cache trends do not have: Query Store may be OFF on every database. Same `source` / `effective_start` / `window_truncated` / `bucket` block as its siblings, with `bucket` `per-interval` | `server_name`, `hours_back`, `as_of` |
        ...
        | `get_lock_wait_trend` | The whole LCK% family's wait milliseconds per SECOND, summed across types and bucketed, with a `wait_types` legend of the types that waited — the aggregate lock-wait lane. The two trends above count incidents and `get_wait_trend` charts ONE named wait type; this shows lock pressure rising when no single type dominates. An empty result distinguishes a genuinely quiet window (`empty`, widen `hours_back`) from a server no wait stats have ever been stored for (`unavailable`), which is NOT a report of a server without lock contention | `server_name`, `hours_back`, `as_of`, `bucket_minutes` |
        ...
        | `get_file_io_trend` | Read/write latency over time per database and file type, the five with the most I/O stall on their own lines and the rest in one `(other)` line; `database_name` charts one database per file. An empty result distinguishes a quiet window (`empty`, widen `hours_back`) from a server nothing has ever been collected for (`unavailable`, collection is not running) | `server_name`, `hours_back`, `as_of`, `bucket_minutes`, `database_name` |
        ...
        Every one of the nine carries a SOURCE WITNESS: `source_observed` (whether this server's system_health session has EVER been read into the store — any event of any type) and `last_captured_at` (the collector's newest capture). Zero rows is four different answers and the read says which: captured in the window and gated out (`empty`, `events_in_window` > 0 — the healthy one), captured before but not in this window (`empty`, `last_captured_of_type_at` says when — widen), never captured for THIS category while the session is being read (`empty`, `source_observed` true — for a rare category such as a memory-node OOM this is the measurement, not a gap), and nothing of any type ever (`unavailable`, `source_observed` false — a dead session or a collector that never ran, and NOT an all-clear). Zero is a measurement only when the source was observed.
        ...
        | Tool | Purpose | Key Parameters |
        |------|---------|----------------|
        | `get_active_queries` | Active query snapshots from sys.dm_exec_requests — what was running at each collection point. `database_name` / `blocking_only` are applied IN the query; `total_snapshots` counts the filtered population, `snapshots_returned` the page, `truncated` says the population held more. Head blockers a victim names are never stripped (`is_head_blocker`); a victim whose blocker is absent says why in `blocker_not_shown` (`not_captured` / `filtered` / `past_page`) | `server_name`, `hours_back` (default 1), `database_name`, `blocking_only`, `limit`, `as_of` |
        ...
        | `get_analysis_facts` | Exposes raw scored facts from the collect+detect+score pipeline — every observation the engine sees, the anomaly detector's ANOMALY_* facts included, with base severity, amplifiers, and metadata (an ANOMALY_* fact's `baseline_confidence` is the baseline's trustworthiness, not a finding's `confidence`) | `server_name`, `hours_back` (default 4), `source` (one of the engine's 15 registered sources — anomaly, bad_actor, blocking, config, coverage, cpu, database_config, disk, io, jobs, memory, queries, sessions, tempdb, waits; an unknown value is refused with the set), `min_severity`, `as_of` |
        | `compare_analysis` | Compares two time periods (e.g., peak vs off-peak, yesterday vs today, the windows around a change), banding each fact worse / better / stable by how far its VALUE moved on the server's own scale — in the stored per-server baseline's robust sigma where one exists (`delta_sigma`, `band_source` `baseline`), otherwise only when the value moved at least a quarter of the larger side AND registers a quarter of the way up its own severity ladder (`band_source` `absolute`); `band_rules` states the rules on every payload. Rows are grouped into physical-cause `families` (one I/O stall is one family row), and `BAD_ACTOR_<hash>` appearances are `plan_cache_churn`, not new or resolved issues. A verdict is a DIFFERENCE, not an experiment: same-hour-yesterday at N=1 vs N=1 cannot show that a change caused anything. When NEITHER window produced facts the result is `unavailable` rather than an all-zero comparison, because "nothing to compare" is not "nothing changed"; when only ONE window is empty the payload carries a `caveat` saying so; a partly collected side flags every verdict row with `coverage_caveat`. `baseline_hours_back` is measured from the comparison window's END, so `as_of` moves BOTH windows together | `server_name`, `hours_back` (default 4), `baseline_hours_back` (default 28), `as_of` |
        | `audit_config` | Configuration audit: evaluates CTFP, MAXDOP, max memory, and max worker threads against best practices and the server's resources (edition is reported, not consulted) | `server_name` |
        ...
        | `mute_analysis_finding` | Mutes a finding pattern by story_path_hash so it won't appear in future runs. Idempotent per scope: the same hash muted twice in the same scope writes nothing the second time (status `already_muted`, `registered` false). Reports what the write did: `registered`, `already_muted`, `story_path` (the chain the row names, null when no retained finding carries the hash), and `matched_now` — how many stored findings in scope carry the hash (status `muted_unmatched` when 0: the mute is kept, but check the hash) | `story_path_hash` (required), `server_name`, `reason` |
        ...
        7. **Config**: Use `audit_config` for resource-aware configuration recommendations (CTFP, MAXDOP, max memory, worker threads)
        ...
        A tool that caught an exception answers with the `{"status":"error","message":...}` envelope described under "Reading an empty result" — the message carries the cause, and `hints.operation` names the tool. A tool that refused the call answers with the `{"status":"invalid","message":...}` envelope from the same section — `hints.parameter` names the knob to fix. The messages below are the `message` of a refusal or a miss, not failures:
        - "Could not resolve server" — `invalid`: server name not found; the message lists the available servers, or use `list_servers`
