...
SELECT
    ...
    /* #2515: the growth ceiling, and MIN is the right aggregate for it twice over. -1 (at least one
       ...
       rows — collected before the v56 migration — are skipped rather than dragging the answer down. */
...
AND   collection_time <= $3";
    ...
    private async Task CollectFileAutogrowthFactsAsync(AnalysisContext context, List<Fact> facts)
    {
        try
        {
            ...
            /* #3896: bounded to the lookback like the database-size read (it had no time bound at all, so a
               dropped database's file counted for the whole database_size_stats retention), and on the same
               bounds as the drill-down that lists these files (DrillDownCollector.CollectAutogrowthPercentFiles). */
            using var cmd = connection.CreateCommand();
            cmd.CommandText = @"
WITH latest AS (
