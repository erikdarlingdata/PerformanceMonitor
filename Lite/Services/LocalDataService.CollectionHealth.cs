...
public class CollectorHealthRow
{
    ...
    /// reads as the daily recapture interval instead of the floor thresholds a raw 0 used to fall to. This
    /// is the same substitution the scheduler and the analysis pass already make for every other consumer
    /// of a resolved cadence, and it is what lets <see cref="CollectorHealthClassifier.Classify"/> band an
    /// on-load collector on the SAME ladder as any other. The banding uses the shipped default, not the
    /// per-install ScheduleManager override, so all three surfaces stay in parity. Internal since #2296:
    /// the tool's sweep-pressure roll-up amortizes each collector's average duration by this same cadence,
    /// so both readers of it share one resolution.</summary>
