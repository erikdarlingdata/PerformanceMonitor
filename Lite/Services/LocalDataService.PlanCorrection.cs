...
public class PlanCorrectionRow
{
    ...
    /* sys.dm_db_tuning_recommendations reports these four in UTC, so they take the same UTC renderer
       CollectionTime above takes; every timestamp in this row is in one frame. The frame is measured
       against the collector-written UTC collection_time rather than inferred from PlanCorrectionCollector
       shipping the DMV verbatim — DarlingPlanCorrectionReader's remarks carry the measurement. */
