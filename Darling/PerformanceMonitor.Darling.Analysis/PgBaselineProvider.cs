...
public class PgBaselineProvider
{
        ...
        int serverId, string metricName, DateTime analysisTime, string query, CancellationToken cancellationToken)
    {
        ...
        {
            /* #2299: a stop mid-computation is not this metric's fault - the postmaster going down
               (57P01) and the disposed data source are the residue of the stop itself. Rethrow as the
               cancellation the pass already handles, so one INFO line replaces five of these. Only
               with the token signalled - the same shapes mid-run stay ERRORs below. */
