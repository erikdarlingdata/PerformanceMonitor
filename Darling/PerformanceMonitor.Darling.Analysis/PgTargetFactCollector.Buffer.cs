...
{
    /// <summary>
    /// <c>PG_BUFFER_CACHE_PRESSURE</c>: the composite of <c>pg_database_stats</c> hit ratio, <c>pg_io_stats</c>
    /// evictions (PG 16+; metadata <c>evictions_tracked = 0/1</c>) and bgwriter counters, with the hit-ratio arm
    /// suppressed on Aurora (<c>hit_ratio_suppressed</c> — the storage / cache split the V63 note describes).
    ...
    /// </summary>
    private partial Task CollectBufferFactsAsync(AnalysisContext context, List<Fact> facts) => Task.CompletedTask;
}
