...
{
    /// <summary>
    /// <c>ANOMALY_PG_REPLICATION_LAG</c>: the window's peak replay lag (bytes, worst standby, from <c>pg_replication_stats</c>) against the <c>pg_replay_lag_bytes</c> bucket — the z-score shape, graded by the shared deviation ramp. The 5-minute cadence: the fact states its own sample count.
    /// <para>/* filled by lane 12 — returns immediately until then. The filled body copies the five v1 detectors
    ...
    /// </summary>
    private partial Task DetectReplicationAnomalies(AnalysisContext context, List<Fact> anomalies) => Task.CompletedTask;
}
