...
{
    /// <summary>
    /// <c>PG_CONNECTION_SATURATION</c> from <c>pg_session_states</c> denormalised totals, or
    /// <c>PG_MONITORING_PERMISSIONS</c> when the <c>state_is_redacted</c> share says the monitoring role could not
    /// see session state. Pattern: <c>DarlingPgSessionStatesReader.PgSessionStatesSql</c>.
    /// <para>/* filled by lane 3 — returns immediately until then. The SQL it will run is declared as a
    ...
    /// </summary>
    private partial Task CollectSessionFactsAsync(AnalysisContext context, List<Fact> facts) => Task.CompletedTask;
}
