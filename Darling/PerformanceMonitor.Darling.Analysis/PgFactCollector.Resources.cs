...
{
    /* #3896: $2 is the lookback start (AnalysisContext.LatestValueStart). The ordered LIMIT 1 already stops at
...
LIMIT 1";
    ...
    private async Task CollectRunnableTaskFactsAsync(AnalysisContext context, List<Fact> facts)
    {
        try
        {
            ...
            using var cmd = new NpgsqlCommand(RunnableTaskStatsSql, connection) { CommandTimeout = FactCommandTimeoutSeconds };
            cmd.Parameters.AddWithValue(context.ServerId);
...
AND   collection_time <= $3";
    ...
    private async Task CollectMemoryGrantFactsAsync(AnalysisContext context, List<Fact> facts)
    {
        try
        {
            ...
            using var cmd = new NpgsqlCommand(MemoryGrantSql, connection) { CommandTimeout = FactCommandTimeoutSeconds };
            cmd.Parameters.AddWithValue(context.ServerId);
            cmd.Parameters.AddWithValue(AsNaive(context.TimeRangeStart));
            cmd.Parameters.AddWithValue(AsNaive(context.TimeRangeEnd));
    ...
    /* #3896: $2 is the lookback start (AnalysisContext.LatestValueStart); unbounded, the window function
...
AND   collection_time <= $3";
    ...
    private async Task CollectCpuUtilizationFactsAsync(AnalysisContext context, List<Fact> facts)
    {
        try
        {
            ...
            using var cmd = new NpgsqlCommand(CpuUtilizationSql, connection) { CommandTimeout = FactCommandTimeoutSeconds };
            cmd.Parameters.AddWithValue(context.ServerId);
            cmd.Parameters.AddWithValue(AsNaive(context.TimeRangeStart));
            cmd.Parameters.AddWithValue(AsNaive(context.TimeRangeEnd));
    ...
    /* #3527: delta_cntr_value spans one COLLECTION INTERVAL, not one second — at a 60s cadence the raw
       delta is 60x the true rate. The honest rate divides by the row's MEASURED sample_interval_seconds
...
FROM latest WHERE rn = 1";
    ...
    private async Task CollectPerfmonFactsAsync(AnalysisContext context, List<Fact> facts)
    {
        try
        {
            ...
            using var cmd = new NpgsqlCommand(PerfmonSql, connection) { CommandTimeout = FactCommandTimeoutSeconds };
            cmd.Parameters.AddWithValue(context.ServerId);
            cmd.Parameters.AddWithValue(AsNaive(context.TimeRangeStart));
            cmd.Parameters.AddWithValue(AsNaive(context.TimeRangeEnd));
...
WHERE rnk = 1";
    ...
    /// like CollectMemoryFactsAsync: the newest snapshot within <see cref="AnalysisContext.LatestValueLookback"/>
...
AND   collection_time <= $3";
    ...
    private async Task CollectMemoryPressureEventFactsAsync(AnalysisContext context, List<Fact> facts)
    {
        try
        {
            ...
            using var cmd = new NpgsqlCommand(MemoryPressureEventsSql, connection) { CommandTimeout = FactCommandTimeoutSeconds };
            cmd.Parameters.AddWithValue(context.ServerId);
            cmd.Parameters.AddWithValue(AsNaive(context.TimeRangeStart));
            cmd.Parameters.AddWithValue(AsNaive(context.TimeRangeEnd));
