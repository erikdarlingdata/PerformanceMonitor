...
{
    /// <summary>
    /// <c>ANOMALY_PG_IO_LATENCY</c>: the window's peak read latency (ms per read, from <c>pg_io_stats</c>) against the <c>pg_io_read_latency</c> bucket — the z-score shape <c>DetectSessionAnomalies</c> takes, graded by the shared deviation ramp (registered in <c>PgTargetScorer.IsDeviationScoredAnomalyKey</c>).
    /// <para>/* filled by lane 11 — returns immediately until then. The filled body copies the five v1 detectors
    ...
    /// </summary>
    private partial Task DetectIoAnomalies(AnalysisContext context, List<Fact> anomalies) => Task.CompletedTask;
}
