...
**"TimescaleDB setup failed — continuing in plain-PostgreSQL mode"** (warning) — the extension exists but conversion hit a problem. Everything still works (DELETE-based retention, plain tables); conversion is retried on the next service start. The *availability* half does not wait for that restart: the hourly store-maintenance tick re-probes the extension and logs **"TimescaleDB is available after all"** at Information when it comes back, which restores the store background-job health check (policy-job self-heal, job cadence, Retention Held) that the fallback had switched off (#3815).
...
| Store background-job health (policy-job self-heal across compression, rollup refresh and retention; job cadence; Retention Held), the TimescaleDB availability re-probe and the retention coverage re-evaluation | First sweep after startup, then hourly at :30 past the minute; the availability re-probe runs first and outside the TimescaleDB gate, so a service that fell back to plain-PostgreSQL mode at startup recovers on its own (#3815); a held retention policy arms itself on the first hourly pass after its rollups cover raw, no restart needed (#3812); every pass writes one `Store job health:` line naming what it read, per family (#3816) |
