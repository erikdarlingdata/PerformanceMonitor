...
| **Blocking** (alert) | Blocked-process reports + DMV snapshots, `blockingCountThreshold` | Distinct root blockers in a periodic sample of `pg_stat_activity`, `blocking.pg_count_threshold` (V122) | Both; separate knobs because the denominators differ (engine-recorded reports vs sampled sightings). |
...
| **High CPU** | Ring buffer, 3-sample persistence gate | Performance Insights, banded on ACU headroom, same gate | Both. The SQL Server fire is graded since #3653: WARNING at the configured threshold, CRITICAL at the CPU health band's 95% bar — the card's ladder, not a new number. The PostgreSQL host's fire is not yet graded. |
