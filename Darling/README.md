...
| Tab | Contents |
|---|---|
| **Overview** | A card per registered server (all servers, not the sidebar selection): server name + status dot, CPU (total non-idle with the SQL-only number alongside), memory, blocking and deadlock counts over the last hour, and last-collection time, each colour-banded (CPU ≥ 80% red / ≥ 50% amber / green; blocking and deadlocks red-or-amber when present) with a red **Offline** overlay. Status is never a live ping (the viewer never connects to the monitored servers), but it is more than freshness: the shared `ServerHealthClassifier.ClassifyBand` returns **Offline** when the server has stopped reporting, **Warning** while it is awaiting its first collection, otherwise the worst of the card's own metrics (**Critical** or **Warning**), otherwise **Warning** for a stale collection — the collector-errors flag carries staleness — and only then **Healthy**. So an amber card is one of three unrelated things: a real condition on the server, a server that has not reported yet, or a late sweep. Hover the status word for the sentence naming which ([#2429](https://github.com/erikdarlingdata/PerformanceMonitor/pull/2429)), built from the same metric rows shown underneath so it cannot disagree with them. **Double-click a card** to open that server's tab. Refreshes every 30 seconds |
