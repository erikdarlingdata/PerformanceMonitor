...
```
  [PASS] aurora-writer: PostgreSQL 17 (server_version_num 170007), writer, Aurora — all 29 PostgreSQL collectors apply
  [PASS] aurora-reader: PostgreSQL 17 (server_version_num 170007), reader (in recovery), Aurora — 25 of 29 PostgreSQL collectors apply (skipped: pg_autovacuum_stats, pg_index_bloat, pg_index_usage_stats, pg_table_bloat_stats)
  [PASS] selfhosted:    PostgreSQL 15 (server_version_num 150012), reader (in recovery), not Aurora — 22 of 29 PostgreSQL collectors apply (skipped: pg_autovacuum_stats, pg_cpu_utilization, pg_index_bloat, pg_index_usage_stats, pg_io_stats, pg_table_bloat_stats, pg_wait_stats)
```
...
Every collector declares its own applicability in code (`AppliesTo(CollectorTargetInfo)`), so this is not a hand-maintained per-collector list — the collectors fall into five groups, and a collector outside its supported platform is **skipped before it runs**, not failed and logged every cycle.
...
### procedureStatsPlanCycleInterval (integer, optional)

| Key | Default | Notes |
|---|---|---|
| `procedureStatsPlanCycleInterval` | `4` | How many collection cycles pass between execution-plan captures for `procedure_stats`. `1` captures on every cycle and is byte-identical to releases before this knob existed; `4` captures on one cycle in four. Clamped to `[1,60]`, and a nonsense value (`0` or negative) degrades to `1` rather than to the default, so a bad setting never silently starts skipping plans. **Why it exists:** `procedure_stats` is normally the most expensive collector against a monitored server, and nearly all of that cost is plan XML — a controlled decomposition put **73.8%** of its read loop in RENDERING the plans and **26.0%** in shipping them, against **0.2%** for the same query with no plan apply at all. The render happens inside `sys.dm_exec_text_query_plan`, a server-side TVF, so it is CPU on the **monitored** server rather than on the collector host; skipping a cycle omits the apply entirely rather than rendering and discarding. **What you trade:** plan freshness — a stored plan can be up to this many cycles old, and `4` keeps the worst case inside the collector's own ten-minute candidate window. Runtime statistics (execution counts, CPU, duration, reads) are collected every cycle regardless, and the collector stays on its 1-minute schedule. The capture cycle is offset per server, so a fleet spreads across the interval instead of every server rendering together. Only `procedure_stats` is gated; `query_stats`, `query_store`, `deadlocks` and `blocked_process_report` still capture plans every cycle. A file-only knob (not stored in the control plane): edit and restart. |

### alerts

The shared alert engine's switches and thresholds. Every default mirrors Lite's alert defaults exactly, so an empty section alerts like a fresh Lite install. `enabled: false` turns off all alert evaluation **and** analysis finding notifications (the analysis itself still runs and persists findings).
...
| `blockingCountThreshold` | `1` | Blocked-process count (rolling window) that trips the alert on a SQL Server target |
| `pgBlockingCountThreshold` | `1` | The PostgreSQL twin (#3444, V122): distinct ROOT blockers in the rolling window that trip `Blocking Detected` on a PostgreSQL target, floor 1. Its own key because the two count different things — engine-recorded blocked-process reports there, roots found in a periodic SAMPLE of `pg_stat_activity` here — so moving one deliberately does not move the other. `blockingEnabled` governs both engines. Inert on a store with no PostgreSQL targets |
| `blockingWaitSecondsThreshold` | `0` | Total blocked wait, in seconds, summed across the latest blocking snapshot; `0` = off. A second gate beside the count one, because a count cannot tell one session blocked for an hour from one blocked for a second. Reports as its own "Blocking Wait Time" alert, and unlike the count gate it is level-triggered: it re-fires every cooldown while the wait stays above the threshold and clears when it drops below. Since #3653 the fire sits behind the shared persistence gate as a disjunction: one snapshot at or above **3× the threshold** fires at once, anything between the threshold and that multiple must hold for **3 consecutive collected snapshots** (counted per collection, not per 30 s sweep; a quiet collection between two breaching ones starts a new episode, because the collector writes rows only while something is blocked). The card's `Fired By` field says which arm admitted it (`single_snapshot_3x` / `consecutive_k3`). Measured on one production store class: 97 of 102 episodes were a single snapshot, so a plain consecutive gate would have dropped exactly the severe pile-ups |
| `deadlockEnabled` | `true` | |
| `deadlockCountThreshold` | `1` | Deadlock count (rolling window) that trips the alert on a SQL Server target |
| `pgDeadlockCountThreshold` | `1` | The PostgreSQL twin (#3444, V122): distinct deadlocks in the rolling window that trip `Deadlocks Detected` on a PostgreSQL target, floor 1. Its own key because the SQL Server figure is calibrated against the fleet's deadlock health band (`health_bands.deadlock_warn_per_hour` on `get_alert_settings`), a band a PostgreSQL server does not have. `deadlockEnabled` governs both engines. Inert on a store with no PostgreSQL targets |
...
| `longRunningQueryExcludedProgramNamePrefixes` | `["SQLAgent - TSQL JobStep"]` | Long-Running Query opt-out knob, program arm (#3653): sessions whose `program_name` *starts with* an entry are not evaluated by that alert (case-insensitive, no wildcards; applied ahead of the row cap). Seeded from a 7-day read of one large production store — SQL Agent job steps, ~460 sessions a week. A default, not a constant: `[]` excludes nothing by program |
| `longRunningQueryExcludedLogins` | `["NT AUTHORITY\\SYSTEM", "NT AUTHORITY\\NETWORK SERVICE"]` | Long-Running Query opt-out knob, login arm (#3653): sessions whose `login_name` *equals* an entry are not evaluated (case-insensitive, whole name). Seeded with the two service principals that carried the read's permanent multi-day background; the application's admin login is deliberately not a default (it also runs real ad-hoc long-runners). `[]` excludes nothing by login |

The long-running-query read shape is configurable too, on the same defaults Lite ships: `longRunningQueryMaxResults` (`5`, clamped 1–1000) and the five noise opt-outs `longRunningQueryExcludeSpServerDiagnostics`, `longRunningQueryExcludeWaitFor`, `longRunningQueryExcludeBackups`, `longRunningQueryExcludeMiscWaits`, `longRunningQueryExcludeCdc` (all `true`). Since #3653 the SQL Server read also carries the Long-Running Query **opt-out knob**: sessions whose `program_name` starts with a configured prefix or whose `login_name` equals a configured login are *not evaluated* by the alert at all — not read into the decision, not counted, not fingerprinted, the opposite of a mute rule — and the exclusion is applied inside the read ahead of `longRunningQueryMaxResults`, because the sessions it names are the longest-running on the server by construction and would otherwise fill the cap on every sweep. Its defaults come from a 7-day read of one large production store, whose long-running population was four classes: (1) SQL Agent job steps (`SQLAgent - TSQL JobStep…`, ~460 sessions a week across 11 jobs, medians 35–62 min) — the seeded **program-name prefix**; (2) the `NT AUTHORITY\SYSTEM` / `NT AUTHORITY\NETWORK SERVICE` logins (~70 sessions across 42 servers, medians 4.8–8.6 *days*, CDC-capture shaped) — the seeded **exact logins**; (3) the application's admin login — deliberately *not* a default, because it carries the job wave but also real ad-hoc long-runners, and the job-step prefix already removes its share; (4) named humans — 3 a week, never excluded. Matching is case-insensitive with no wildcard grammar; the fired card's `Excluded Count` item splits the sessions removed by arm (a session matching both arms counts once, under the prefix). **On Darling the knob lives on the settings row since V135**: two `text[]` columns on `config_alert_settings` (`longRunningQueryExcludedProgramNamePrefixes` / `longRunningQueryExcludedLogins` in `darling.json`, seeded on first run; the store row wins afterwards like every alert knob), edited in the Viewer's Settings window or through `update_alert_settings`, reported by `get_alert_settings`; a pre-rung row reads as the seeds, and a list an operator clears stays cleared. Lite ships the same defaults in `settings.json`. The PostgreSQL twin consults the same knob through the same shared predicate builder (#3743): `application_name` is the `program_name` twin and `usename` (stored as `username`) the `login_name` twin, so an entry means the same thing on both engines — prefix for programs, whole name for logins, case-insensitive — applied ahead of the row cap, with the fired card's `Excluded Count` split rendered the same way. The seeds name SQL Server internals and match nothing on PostgreSQL by construction; whether that engine wants seeds of its own is a production read, not a guess. On a PostgreSQL target the read also honours `longRunningQueryExcludeBackups` (it drops `pg_dump` / `pg_dumpall` / `pg_restore` / `pg_basebackup` sessions by `application_name`) and `excludedDatabases` through the same builder's database arm, ahead of the same row cap (#3742) — the PostgreSQL twin applies all three exclusions in the read, so an excluded database's sessions can no longer consume the page, and the fired card carries an `Excluded By Database` count beside the knob's when the list is set — and unconditionally skips non-client backends (autovacuum workers, walsenders) and the `VACUUM` / `ANALYZE` / `REINDEX` / `CLUSTER` statements; the other three switches name SQL Server internals with no PostgreSQL reading and are ignored there — see the engine-coverage table below. Not configurable: the analysis-finding notification policy (notify at severity >= 1.5, 6-hour per-finding cooldown).

#### Which engine each built-in alert and health band covers

One alert name means one condition on both engines wherever both engines can honestly measure it; where only one engine can, the row says why rather than leaving the gap to be discovered (#3539). The health bands are the fleet card's dots (`get_fleet_overview`, `/api/fleet`, the viewer's Overview).

| Condition | SQL Server | PostgreSQL | Why one-sided, where it is |
|---|---|---|---|
| **Deadlocks** (alert) | Extended-event graphs, `deadlockCountThreshold` per rolling window | Server-log deadlocks parsed into `pg_deadlocks`, `deadlocks.pg_count_threshold` (V122) | Both. Separate count knobs because the instruments differ (a captured graph vs a parsed log entry) and an operator tuning one should not silently move the other. The SQL Server fire is **graded** since #3653: WARNING by default, CRITICAL when the hour's rate reaches the health band's `deadlock_critical_per_hour` tier below — the count knob decides whether it fires, the band's pair decides the colour, so the alert row and the fleet card agree about the same hour. The PostgreSQL host's fire is graded the same way since #3653 — the same grader, the same one-hour count, the same store tiers — on its own deduped report count (the number the row quotes), not on the counter difference the band row below uses; the read caps at 50 distinct reports, so a Critical tier raised past 50/hr is unreachable on that host. |
| **Deadlocks** (health band) | `v_deadlocks` graphs per hour | `pg_stat_database.deadlocks` — the server's own lifetime counter, differenced between consecutive samples per database, clamped at zero across a statistics reset, summed across the cluster's databases | Both, through the **same** `health_bands.deadlock_warn_per_hour` / `deadlock_critical_per_hour` tiers: a deadlock per hour is the same quantity whichever engine recorded it. The PostgreSQL arm bands only when at least one difference was taken in the window (a difference of fewer than two samples is not a zero), so a target nothing has collected from reads Unknown rather than Healthy. The fleet total's `deadlock_coverage.postgres_servers` names how many servers were counted the counter way; `get_pg_deadlocks` has the graphs. Never `SUM(deadlocks)` on the store — the column is a lifetime counter repeated in every one-minute sample. |
| **Blocking** (alert) | Blocked-process reports + DMV snapshots, `blockingCountThreshold` | Distinct root blockers in a periodic sample of `pg_stat_activity`, `blocking.pg_count_threshold` (V122) | Both; separate knobs because the denominators differ (engine-recorded reports vs sampled sightings). The PostgreSQL fire carries an explicit WARNING tier and no CRITICAL tier (#3653): the band row below says why no measured bar exists for sampled sightings, and a fire bar is not a tier. |
| **Blocking** (health band) | Reports per hour + longest block, the #3596 tiers | **Unknown, by design** | The band's count tiers were measured in *reports per server-hour* on engine-recorded evidence. PostgreSQL blocking is SAMPLED once a minute: a waiter seen in three consecutive captures is one wait observed three times, not three reports, so a sighting count through those tiers would band on a denominator they were never measured against. An honest PostgreSQL band needs its own sampled-shape tiers (share of captures holding a waiter; longest observed wait) measured on that fleet's distribution, which has not been taken. Unknown-with-reason beats a fake Healthy; the alert speaks for PostgreSQL blocking meanwhile. Tiering pending: #3601's `lock_wait` log events (`log_lock_waits` "still waiting" lines, one per wait past `deadlock_timeout`) are the event-grain evidence a report-rate band would read, and the join point for a future PostgreSQL arm. |
| **Blocking Wait Time** | Sum of blocked wait across the latest live DMV snapshot, `blockingWaitSecondsThreshold`; since #3653 gated: 3× the bar on one snapshot, or 3 consecutive collections | — | SQL-only. The condition is a *live total* of how long sessions are currently blocked, which the DMV snapshot gives exactly. `pg_blocking` is a per-minute sample of `pg_stat_activity`: summing observed waits across one capture is a lower bound on the same quantity taken at an arbitrary instant, and a threshold in seconds against a once-a-minute sample would fire on the sampling grain as much as on the workload. A sampled twin is possible but needs its own stated denominator; not faked under the SQL Server name. |
| **Long-Running Query** | `sys.dm_exec_requests`, five noise opt-outs (sp_server_diagnostics, WAITFOR, backups, misc XE waits, CDC) + `session_id > 50` + `excludedDatabases` + the #3653 opt-out knob (`program_name` prefixes / exact logins, seeded with the SQL Agent job-step prefix and the two `NT AUTHORITY` service logins, applied ahead of the row cap) | Latest `pg_session_states` capture, same threshold and switch | Both. PostgreSQL opt-outs: non-client backends (unconditional — mirrors `session_id > 50`), `VACUUM`/`ANALYZE`/`REINDEX`/`CLUSTER` by command tag (unconditional — SQL Server needs no statement-shape switch because its row carries the text and a mute rule can match it; `pg_session_states` stores no text, so the tag is the only handle), dump/restore utilities by `application_name` on the shared `longRunningQueryExcludeBackups`, the #3653 opt-out knob over `application_name` / `usename` through the same shared predicate builder and ahead of the same row cap (#3743; the SQL Server seeds match nothing here), `excludedDatabases` as the same CTE's third flag ahead of the same cap (#3742). `CREATE` is reported (with its tag) because the tag cannot tell an index build from `CREATE TABLE AS`. The remaining three SQL Server switches name SQL Server internals; `pg_sleep` (WAITFOR's cousin) is invisible without text. The PostgreSQL fire carries an explicit WARNING tier and no CRITICAL tier (#3653): the only bar the alert has is the operator's threshold, and no measured duration distribution exists on either engine to cut a second tier from. |
| **Poison Wait** | THREADPOOL / RESOURCE_SEMAPHORE / RESOURCE_SEMAPHORE_QUERY_COMPILE | IPC:BtreePage / IPC:BufferIo (Aurora) | Both, **one shape** since #3593: accumulated wait over a rolling ten-minute window, graded WARNING/CRITICAL from the shared `PoisonWaitEvaluator` bars on both engines. The earlier divergence (always-CRITICAL on SQL Server, graded on PostgreSQL, under one mute key) is closed. Both hosts clear a standing alert only on an **observed** window (#3593 SQL Server, #3653 PostgreSQL): a collector that stops delivering holds the alert open rather than announcing a recovery nobody measured. On PostgreSQL the witness is the `pg_wait_stats` collector's own `collection_log` runs inside the window, because since #2694 a quiet poison event writes no row and the rows alone cannot tell a finished storm from a dead collector. |
| **High CPU** | Ring buffer, 3-sample persistence gate | Performance Insights, banded on ACU headroom, same gate | Both. Both fires are graded since #3653: WARNING at the configured threshold, CRITICAL at the CPU health band's 95% bar — the card's ladder, not a new number. On PostgreSQL the grade is the band's own classifier on the same two figures the card is handed, so the 95% is of the configured ACU ceiling (the figure the gate thresholded), never of the currently allocated capacity. |
| **Database File Growth** | Per-file growth over the lookback window vs `FileGrowthRiseMb` | — | SQL-only. It is a SQL Server *file-model* condition: data/log files with explicit growth increments and ceilings, where a rise means a file autogrew. PostgreSQL has no file growth to watch — relations grow a page at a time and the disk-level answer is Volume Free Space. The PostgreSQL relative is *bloat growth* (`pg_table_bloat_stats` hourly and `pg_index_bloat` daily are collected); a "bloat growth" alert is a future item and would not share this name, because it answers a different question. |
| **TempDB Space**, **Volume Free Space**, **Long-Running / Failed Agent Job** | Yes (TempDB Space: 3-sample persistence gate) | — | SQL-only by construction (tempdb, host volumes via `sys.dm_os_volume_stats`, SQL Agent). TempDB Space sits behind the same shared persistence gate as High CPU since #3653 (A5): 3 consecutive collected `tempdb_stats` samples at or above the threshold to fire, the first collected sample below it to resolve, counted per collection rather than per 30 s sweep, and a reading that stops arriving freezes the gate rather than resolving it — measured on a 43-server production fleet, every one of one server's 22 crossings in 14 days was 1–4 samples long. TempDB Space fires with an explicit WARNING tier and no CRITICAL tier (#3653): the product has no measured "tempdb nearly full" bar to cite, and the percent's denominator is the growth ceiling where one exists and the allocation where not, so a single reserved-percent tier would mean two distances from failure. |
| **PostgreSQL Wraparound Risk**, **PostgreSQL Vacuum Horizon Blocked** | — | Yes | PostgreSQL-only by construction: MVCC transaction-id wraparound and the xmin horizon have no SQL Server counterpart. |
| **PostgreSQL Replication Slot Retention** | — | `pg_replication_slots` retained WAL, `wal_status` | The SQL Server analogue is a database whose log cannot truncate — `sys.databases.log_reuse_wait_desc` reading REPLICATION / AVAILABILITY_REPLICA / ACTIVE_TRANSACTION / LOG_BACKUP for consecutive collections. That column IS collected, but only as the load-time `database_config` snapshot (frequency 0), not as a time series, so a persistence-gated "Log Reuse Blocked" condition cannot be evaluated honestly today; it needs the column on a cadenced per-database collector (`database_states` is the natural home), which is a collector change with its own census and pins. Named here so the pair is on record: slot retention on PostgreSQL, log-reuse wait on SQL Server. |
| **Server unreachable**, **Collection stopped**, **Retention Held**, collector-health bands | Yes | Yes | Both — these are about the monitor, not the engine. |

**Same-statement pileup** is the one analysis finding evaluated on the COLLECTION cadence rather than the analysis interval, because the shape it describes does not survive a 30-minute wait: N concurrent sessions executing one statement, every copy past 10 seconds, at least one suspended on an IO-class wait, where that statement's own snapshots over the prior 45 minutes were sub-second — the live signature of a shared parameterized plan flipping to an IO-heavy shape, which takes every caller down together and is usually over in about ninety seconds. It is computed from the active-query snapshot collector alone, so it works on deployments with the Query Store readers disabled, and it rides the notification policy above unchanged (severity >= 1.5 pages; the severity scales with concurrent sessions × elapsed, so a brief threshold pileup persists as a visible finding without paging). Episodes of the same statement share one incident id, so a recurrence folds onto the same trail as an occurrence rather than arriving as a fresh surprise — and, like every standing condition, it does not re-page inside the cooldown unless it worsens. Unlike the scheduled pass it has no 24-hour data-span requirement: its evidence is the snapshot plus that statement's own recent history, so it works on a young install.

### analysis

Scheduled analysis: the triage engine runs per server on a cadence, persists its findings to `analysis_findings` (the Recommendations surfaces, `get_analysis_findings` and the web dashboard read them the instant they land) and, independently, notifies on the ones that cross a severity floor. The first four keys are control-plane knobs (`config_alert_settings`, editable from the Viewer and `update_alert_settings`); the last is file-level.

| Key | Default | Meaning |
|---|---|---|
| `enabled` | `true` | Run the scheduled analysis pass at all |
| `intervalMinutes` | `30` | Per-server cadence (clamped 5–360) |
| `notificationsEnabled` | `true` | Deliver findings to the channels at all (analysis still runs and persists with this off) |
| `notifySeverity` | `1.5` | The severity floor (0–2) a finding must reach to be notify-worthy |
| `uncorroboratedRoute` | `"digest"` | **Where a notify-worthy but UNCORROBORATED finding goes** (#3712). `"digest"` keeps it off email, Teams, Slack, PagerDuty and the generic webhook: it is still persisted, still visible on the web dashboard and MCP the moment it fires, recorded in the alert history as `notification_type: digest` with the gate's reason (`routing_reason` on `get_alert_history`), and named once a day in the **Analysis Singles Digest**. `"page"` restores the pre-#3712 behaviour where every finding at or above `notifySeverity` pages. **The file half of a two-home knob** since V137: `config_alert_settings.analysis_uncorroborated_route` (nullable, CHECK `digest`/`page`) is the store half, and the running service resolves **store non-NULL wins over file** — a route set in the Viewer's Settings → Automated Analysis combo or through `update_alert_settings` is live within one collection sweep and wins over this value; NULL there (every store after the V137 upgrade, and every fresh seed — the seed deliberately does not copy this value in) defers to this key, read once at service start; a value here that is neither spelling defers to the shipped `digest`. `get_alert_settings` reports the effective route and which home decided it (`analysis.uncorroborated_route_source`: `store` / `file` / `default`); sending `analysis.uncorroborated_route: null` to `update_alert_settings` clears the column so this key governs again. |

**Confidence chooses the channel.** A finding earns a page by *corroboration*, never by severity alone: two or more facts in its story chain (each traversed edge is a fired predicate onto a fact that itself scored), or a matched co-fire check on its root fact (every check the scorer defines is a predicate over ANOTHER fact — a sibling anomaly in the same window, an absolute fact at its concerning bar, a wait that is significant), or one of the two by-construction stories (the absolution, the same-statement pileup). A lone fact — one fact, no matched check — is a *single*, and singles route to the digest under the default. The gate reads those components, not the confidence scalar, so a future re-weighting of the confidence formula cannot move the paging bar by itself. The extremity escape keeps its severity power (an extreme single still clears the floor and still leads its story), but its published intent — extreme *and* corroborated — is the enforced bar for a page. The measurement behind the default: the first day the recalibrated engine ran on a large production fleet it paged forty-plus times in seven hours on anomaly singles at severity 1.5–1.9 and confidence 0.20–0.35, interleaved with the handful that needed a human; most were true and redundant with a page something else had already sent.

**Escalation is the corroboration event.** A single that later gains a fact or a matched check pages at that moment as a NEW firing — the digest route cools in its own key namespace and digest rows are excluded from the page cooldown's restart seed, so an earlier digest entry can never be the cooldown that eats the page. An uncorroborated single riding in an incident with a corroborated finding is named on that page as co-fired, exactly as before.

**The Analysis Singles Digest** is the third daily document beside the Collector Cost Digest and the Fleet Sweep Rollup (family `reports`, INFO, one post per trailing day, delivered-today gate in the store, empty day posts nothing): "Anomaly singles: N across M servers", the top movers by severity and a by-family, by-server breakdown, each line carrying the same alert-history key (`Analysis: <category> [<hash>]`) and #1140 dedup fingerprints a page would have carried, so any single can be promoted to a full drill-down without archaeology.
...
#### Notification routes — sending alert families to different channels

The channels above (and the SMTP, PagerDuty and generic-webhook ones the Viewer's Settings window adds) are the **parent** channel set: one destination per channel type, and by default every alert goes to every configured one. One stream then interleaves four audiences — the monitor's own health, scheduled reports, Agent job failures, and the performance pages the channel exists for — and the reader routes in their head on every post. Since V131 (#3598) a sparse **routes** table layered over the parent row lets the operator make that call once: **Settings → Notifications → Manage Notification Routes…**, or read it back with `get_notification_routes`.

A route names what it matches and a destination per channel type, and **an empty channel on a route inherits the parent's** — so a route can point one family's Slack at a second webhook and leave PagerDuty exactly as the parent has it, or put a PagerDuty key on the `performance` route *alone* with none on the parent, which is how "only pages page" is spelled. What a route matches is either a **family** or one **exact metric name** (as `metric_name` reads in Alert History; an exact route also catches that alert's recovery — `Server Restored` routes as `Server Unreachable`, `AG Replica Reconnected` as `AG Replica Disconnected`). The four families are a closed taxonomy the product writes down (`AlertFamily`), pinned by a census so a new alert cannot ship unclassified:
...
- **Diagnostic-analysis tools** — `analyze_server`, `get_analysis_facts`, `compare_analysis`, `audit_config`, `get_analysis_findings`, `mute_analysis_finding`.
...
- **Core data-read tools** — the diagnostic reads an assistant needs to investigate a server, each a stored read of the collected data (never a live query against the monitored server):
  ...
  - *Query Store clutter* — `get_query_store_clutter` (#3797), composed from rows already collected rather than from a new query against the server: per database the `query_store` collector's read cost (how often and by how much it was the slowest fan-out item, off `collection_log`'s V80 rollup), plan churn (`plan_id` cardinality on the raw `query_store_stats` rows — plans per query at p95, arrivals per day, the one-shot fraction) and the newest `query_store_health` options row, each with raw numbers and a decomposed `verdict`; plus ONE per-server overhead block — the non-sleep `QDS_*` wait deltas with the four sleep waits `IgnoredWaitDefaults` drops named as excluded, and `MEMORYCLERK_QUERYDISKSTORE`. A database whose `readonly_reason` carries the readable-secondary bit is excluded by architecture with the reason on the row (a replica's Query Store is its primary's); `query_capture_mode` is the V137 (#3796) column read verbatim, with `capture_mode_known` beside it — a null there is a capture older than the rung, never `NONE` — and the churn recommendations name the mode (`ALL` beside churn is the switch-to-`AUTO` case, `AUTO` beside it points at the workload); `fleet_median` is opt-in because it walks two raw hypertables fleet-wide. The same rows reach the web server page's **Queries** tab (one fetch, three panels: the per-database rows, the per-server wait block and the memory clerk) and the Viewer's **Queries → Query Store Clutter** sub-tab, both reading the same statements and the same composition, so a verdict and a remedy are worded once.
  - *Discovery / health* — `list_servers` (with collection-freshness status, each row's engine — `engine_kind` is the registry token, `engine_version` the engine-aware version label, and `sql_version` a **deprecated** alias of `engine_version` kept so existing consumers keep working — and the [declared peer stores](#peers) when the fleet is split across several Darling boxes), `get_collection_health`, `get_server_properties`.
...
- **Diagnostic-depth data-read tools** — deeper reads for a blocking / deadlock / session / configuration / storage investigation, each a stored read:
...
- **Resource-contention + jobs data-read tools** — deeper reads for an internal-contention / worker-thread / plan-cache / SQL Agent investigation, each a stored read of the latest collected snapshot:
...
- **Trend data-read tools** — windowed time-series siblings of the core reads, each a stored read of the collected series over the window (BOTH-sides, naive-UTC):
...
- **Alert + health-overview tools** — the fleet-triage reads the fleet edition previously lacked, each a stored read over the monitoring store (no live hit):
  - *Alerts* — `get_alert_history` (what fired, value vs threshold, delivery success/failure, muted, and since V131 `route` — where the post went — fleet-wide by default, or scoped to a server), `get_alert_settings` (the current alert config the service is using — per-alert enable/thresholds, cooldown, excluded databases, delivery mode, analysis cadence), `get_mute_rules` (the alert mute rules in force, so a suppressed server is distinguishable from a healthy-quiet one), `get_notification_routes` (the alert-family taxonomy and the [notification routes](#notification-routes--sending-alert-families-to-different-channels) — which channels each route sets, never the destination values).
...
- **Fleet sweep reports (Darling-only)** — `get_sweep_reports` reads the scheduled [Fleet Sweep Reports](https://github.com/erikdarlingdata/PerformanceMonitor/issues/3466) — the stateful whole-fleet summaries the service persists at the configured cadence (`get_alert_settings`' `fleet_sweep` group; default hourly): the sweep timeline over a caller-set window with each sweep's document embedded, the newest sweep in full (per-server verdicts with band transitions pre-computed, and the would-have-paged ledger whenever a sweep ran under the alert master switch — present-and-empty on a muted sweep with nothing to report, ABSENT on an alerts-on sweep where no such check was made), and the watch-item worklist with its hysteresis position. One `sweep_id` (as a STRING — the ids are too large for a JSON number to round-trip) fetches one sweep in full. The same `FleetSweepPresentation` builders and store reads the web feed's `/api/sweeps` routes serve, so an agent and a browser cannot disagree about what a sweep looks like. The description carries the two field contracts an agent must read: `alerts_enabled: false` on a sweep means DELIVERY WAS OFF while it ran, and `instruments_alive: false` means the sweep could not prove its own data sources — quiet is not clean, and the `instrument_liveness` block says which instrument went unproven. A stored read; no monitored server is touched.
...
- **Alert-tuning write tools (Darling-only)** — `update_alert_settings`, `create_mute_rule`, `update_mute_rule`, `delete_mute_rule`, `set_mute_rule_enabled`, and the two route writes `set_notification_route_enabled` / `delete_notification_route` (a route leaves force or is removed; authoring one — a destination URL — is the Viewer's Settings window only, because a destination is a bearer secret the `mcp` role never holds) let an MCP client TUNE the alert engine the fleet shares — the SAME config `get_alert_settings` / `get_mute_rules` read and the Viewer's Settings window writes. `update_alert_settings` is a PARTIAL update of the alert configuration: read via `get_alert_settings`, change fields, and send only those back in the same nested shape; every field is validated against the SAME ranges/enums the Settings window enforces BEFORE any write, an out-of-range or unknown field returns `{status:"invalid"}` and writes nothing, and the write self-bumps `config_version` so the running service hot-reloads within one collection sweep. `create_mute_rule` / `update_mute_rule` / `delete_mute_rule` / `set_mute_rule_enabled` reuse the SAME `PgMuteRuleStore` `get_mute_rules` reads through (and the same GUID id-generation the Viewer's mute-create path uses). `set_mute_rule_enabled` is the headless twin of the Viewer's mute-rule checkbox: it writes `enabled` and nothing else, so a rule can leave force and return keeping its id, its reason and the `created_at_utc` the `Stale Mute Rules` self-alert ages it from — where a delete-and-re-create resets that clock and delivers the suppressed alerts in between. `update_mute_rule` is the headless twin of the Viewer's Edit dialog on the same terms: a PARTIAL update (send only the fields to change, in the shape `get_mute_rules` returns; an explicit JSON `null` clears a field, so a bounded rule can be made permanent again) written through the store's own full-row `UpdateAsync` — whose SET list does not name `created_at_utc`, so no edit, however sweeping, resets the staleness clock the way the delete/recreate workaround did for every field the enable flag cannot express. `update_alert_settings` spans the two singleton config rows the alert engine reads: every knob is on `config_alert_settings` except the per-fingerprint DELIVERY cooldown, which is stored as `config_notification.email_cooldown_minutes` and reported and accepted as `delivery.cooldown_minutes` — a channel-neutral name, because that one number is the only throttle on a Slack / Teams / PagerDuty / generic-webhook post as well as on email, so a headless deployment with no SMTP configured at all is still governed by it (the stored spelling is accepted as a write-only alias). Note the two cooldowns are different stages: top-level `cooldown_minutes` gates whether the engine FIRES, `delivery.cooldown_minutes` bounds the resulting post — twice over, once per alert FINGERPRINT and once per METRIC across the whole fleet for a RE-notification, so one fault on forty servers does not cost forty posts an hour and the servers that second bound holds back are named on the post that does go out. A first notice is never held back by either bound, and `delivery.mode` `PerEvent` opts out of the per-metric one. None touches a monitored SQL Server or the collected data — only the shared alert configuration; SMTP/webhook delivery credentials are out of scope (the `mcp` role can read and write exactly the one non-secret cooldown column of the notification table, and cannot read or write any secret column). It is part of what the MCP token gates — see [What a token can reach](#opt-in-network-endpoints-lan) below.
...
**What you see.** The dashboard opens on a **Fleet Overview**: a card per enabled server with a status dot, six per-metric health bands (CPU, threads, memory, blocking, deadlocks, collectors), and its last collection time — all banded server-side, so the browser only renders (a server that has never reported shows an amber "Awaiting first collection", never a red offline). Above the cards a worst-first "Needs attention" list surfaces the servers to look at, or an all-healthy line when there is nothing to chase. Click a card to **drill into one server**: an overview, wait stats with a trend for the heaviest wait, active queries, a CPU chart, memory and file-I/O trends, and collection health — the same collected data the viewer shows, over inline charts. A fleet-wide **Alert History** page (with a server filter box) rounds out phase 1, a **Custom Views** page hosts the composer for the saved dashboards/notebooks in `config.custom_views`, and an **Alert Rules** page edits the user-authored custom alert rules. A **Fleet Sweeps** page ([#3466](https://github.com/erikdarlingdata/PerformanceMonitor/issues/3466)) renders the scheduled whole-fleet sweep reports as a timeline at a configurable viewing span (default the last hour): each sweep's document diffs against the previous sweep (band transitions with reasons, arrivals and departures), carries the watch-item worklist with its hysteresis position, states the alert master switch on every sweep and renders the would-have-paged ledger whenever one was taken under mute — and a sweep that could not prove its instruments were alive renders LOUDLY (a red banner and a marked timeline row), because quiet is not clean. The sweep cadence shown there is read from `get_alert_settings`' `fleet_sweep` group; the Settings window and `update_alert_settings` own the knob. Channel delivery of sweep content is bounded at a **daily rollup**, a ceiling rather than a default, per the issue's delivery ruling: at most one INFO post per day through the existing alert channels, summarizing the trailing day's band transitions, watch-item openings and closings, instrument-liveness incidents, and — whenever any covered sweep ran under the alert master switch — the would-have-paged ledger those sweeps carried, so the operator sees what the silence cost. A day with nothing to say posts nothing (the collector-cost digest's own discipline — the web feed is where the affirmative, instrument-proved all-clear lives at full cadence), and the rollup's delivery is itself master-gated: with `alerts_enabled: false` the sweeps keep publishing here and to `get_sweep_reports`, nothing reaches the channel, and the first rollup after re-enable states how many of its covered sweeps ran muted — older muted days stay readable on this page permanently rather than arriving as a re-enable flood. The HTTP API also carries the mute-rule write endpoints (`/api/mute-rules`, [#3450](https://github.com/erikdarlingdata/PerformanceMonitor/issues/3450)) — their page UI is deferred; the endpoints are the deliverable — all writes gated on the editing seat. Otherwise it is a read-only view — no settings, no live-server queries — and it refreshes every 60 seconds (pausing while the tab is hidden). The frontend ships fully self-contained (no CDN, no fonts, no remote anything), so it works on an air-gapped host with no internet access.
...
**`pg_read_all_data` is what makes the statistics-based reads work, and without it they succeed and store nothing.** `pg_stats` filters every row through `has_column_privilege`, and `pg_monitor` confers no `SELECT` on user tables — so a `pg_monitor`-only role reads `pg_stats` as EMPTY rather than as denied, which is why nothing logs a `PERMISSIONS` skip. THREE collectors change behaviour: `pg_column_stats` stores zero rows on every run, `pg_table_bloat_stats` still collects sizes and dead-tuple counts but suppresses the bloat ESTIMATE that surface exists for (`estimate_unavailable`), and since #3234 `pg_index_bloat` does the same — it records every index at its size with a stated reason and no estimate. **That last one reverses what this paragraph used to say, and the reversal is the point.** Index bloat was measured-only at every grant level, so the grant genuinely did not affect it; #2561 had considered a statistics-based estimator and rejected it precisely because `pg_stats` is invisible to `pg_monitor`. #3234 wrote that estimator and made it the scheduled collector, which moves index bloat onto exactly the dependency the earlier text ruled out. The exact function still works without this grant, which is what the per-row `exact_measurement_command` is for. On Amazon Aurora and RDS the same grant works as an `rds_superuser`. The role is PostgreSQL 14+. On 13 and older it does not exist, so grant per schema instead — `GRANT USAGE ON SCHEMA public TO darling_monitor;`, `GRANT SELECT ON ALL TABLES IN SCHEMA public TO darling_monitor;`, and `ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT SELECT ON TABLES TO darling_monitor;` so tables created later are covered too. `docs/postgres-first-target-runbook.md` carries the same fallback.

**Self-hosted targets add one more pair, for the three collectors that read the server log.** `pg_deadlocks`, `pg_plan_capture` and `pg_log_events` read it with `pg_read_file()`, which `pg_monitor` does not cover: the login needs the `pg_read_server_files` role AND an explicit `GRANT EXECUTE ON FUNCTION pg_read_file(text), pg_read_file(text, bigint, bigint), pg_read_file(text, bigint, bigint, boolean)` — measured on #2566, the role alone does not carry it, because the function's ACL is `postgres=X/postgres`. The EXECUTE half lives in each database's own catalog, so run both grants **in the database Darling connects to**; issued in a different database on the same cluster they change nothing and the failure looks identical (#3239). Unlike the `pg_stats` gap above this one fails LOUDLY — every cycle records a `PERMISSIONS` skip whose message names this exact pair and the database to run it in. On Aurora/RDS neither grant applies or is grantable: there is no filesystem, and both collectors pull the log through the RDS API instead (#2538), authorized by the monitoring host's IAM role rather than any database grant. `docs/postgres-first-target-runbook.md` step 1 carries the sequence and its proof point.

**`pgstattuple` is no longer required by any scheduled collector, and is the ON-REQUEST route to an exact index measurement.** Since #3234 `pg_index_bloat` estimates from catalog statistics and reads no index page, so a database without the extension collects normally rather than failing for that database — which it previously did, observed on the `postgres` maintenance database of the first production target. Every row still reports whether the extension is present, because that decides whether the `exact_measurement_command` it carries needs a `CREATE EXTENSION pgstattuple;` in front of it. That command calls `pgstatindex`, which reads every page of the index and is the analogue of `sys.dm_db_index_physical_stats` in `DETAILED` mode — run it deliberately on the one index you are about to act on rather than on a schedule. It is also the ONLY route for a PARTIAL or a DEDUPLICATED index, neither of which any statistics model can reach: PostgreSQL 13+ stores duplicate keys once in a posting list, so real storage is denser than per-tuple arithmetic allows and a correct model still over-predicts. The extension grants `EXECUTE` to `pg_stat_scan_tables`, which `pg_monitor` already carries — which is why the exact path works on a role the estimate does not.

`pg_stat_statements` must be present for `pg_statement_stats`, which means the extension in `shared_preload_libraries` (a restart, or a parameter-group change plus reboot on Aurora/RDS) and `CREATE EXTENSION pg_stat_statements;` in the database Darling connects to. The extension tracks **all** databases in the cluster keyed by `dbid`, so one installation in the connect database covers the whole instance. The collectors that need something installed, named rather than counted: `pg_statement_stats` (`pg_stat_statements`, above), `pg_buffer_usage` (`pg_buffercache`), `pg_kernel_stats` (`pg_stat_kcache`, which sits on top of `pg_stat_statements`), `pg_predicate_stats` (`pg_qualstats`), and `pg_wait_sampling` (the `pg_wait_sampling` module). Every other collector needs nothing installed: they read core catalogs and Aurora's built-in functions. **Needing `shared_preload_libraries` and a server restart** (a parameter-group change plus a reboot on Aurora/RDS), and inert until then whatever else is installed: `pg_stat_statements`, `pg_stat_kcache`, `pg_qualstats` and `pg_wait_sampling`. **Additionally per database**, because the collectors that read them run per database and `CREATE EXTENSION` in one database does not create them in another: `pg_qualstats`. `pgstattuple` was on this list until #3234 and is now needed only for the on-request exact measurement, by no scheduled collector. Every one of them degrades identically when its dependency is absent — the query raises `42P01` or `42883`, which classifies as `ObjectMissing` and, because the dependency is declared, is recorded as a non-fatal `EXTENSION_MISSING` skip that names the extension (#3240; never as `PERMISSIONS`, whose remedy is a grant) — and `pg_extension_availability` reports which install would light up every one of them except `pg_wait_sampling`. It deliberately does NOT cover `pg_wait_sampling`, which was on that roster and was taken off (#2564, fixed in #2584). The roster is a **reportable-absence** list rather than a list of what collectors depend on, and those are different sets: it carries extensions no collector reads and omits this one, which a collector does read — which is why a dependency is declared on the collector rather than read off the roster. Check `pg_wait_sampling` with `SHOW shared_preload_libraries;` instead.
...
| `pg_statement_stats` | `pg_stat_statements`; `aurora_stat_statements()` on Aurora | 1 min / 30 d | Per-query-shape totals, matching `query_stats`' cadence, wherever `pg_stat_statements` is installed — since #2625 it reads BOTH sources, chosen at query-build time. Aurora's function adds the storage-vs-cache I/O split (`storage_blks_read` / `orcache_blks_hit`) and per-statement peak memory, which core PostgreSQL does not expose; on the vanilla path those columns are written NULL, not zero |
...
| `pg_server_config` | `pg_settings` | 60 min / 365 d | The configuration history — nothing else in the stack can reconstruct one that was never recorded. Hourly because configuration changes when a person changes it, and a year because "this got slow sometime last quarter" is asked long after the fact; `get_pg_server_config_changes` reads consecutive snapshots, so the change and its timing survive the person who made it. `source` is stored so the read can state which rows are server configuration and which are the collector's own session |
| `pg_deadlocks` | the server log via `pg_read_file()`; the RDS log API on Aurora/RDS | 5 min / 90 d | The full deadlock reports PostgreSQL writes to its log — previously only the COUNT (`pg_stat_database.deadlocks`) was collected, a number that goes up. `deadlock_hash` is the identity: the self-hosted route re-reads an overlapping log tail deliberately, so one deadlock lands many times and every read groups on the hash. `graph_text` is stored whole beside the parsed columns, so a lock shape the parser does not break out still arrives readable by a person. Needs nothing enabled at default settings, but `log_error_verbosity = terse` and a non-English `lc_messages` both silently empty it — the readiness collector's `message_locale` facet judges the latter |
| `pg_plan_capture` | `auto_explain` JSON in the server log via `pg_read_file()`; the RDS log API on Aurora/RDS | 60 min / 14 d | Captured execution plans, redacted before storage: query text is dropped outright (identity is `query_id`, whose normalised text `pg_statement_stats` carries) and every remaining string literal is scrubbed, with `plan_hash` taken of the REDACTED plan so a shape recurs to one hash whatever values it ran with. The self-hosted read is a fixed 4 MB tail — #2565 measured 772 MB of log in twenty seconds at capture-everything, so no fixed number bounds the worst case — and a server logging faster than the window covers should lower the cadence per server |
| `pg_log_events` | the server log via `pg_read_file()`; the RDS log API on Aurora/RDS | 5 min / 30 d | The log, CLASSIFIED (#3601): every line at WARNING or worse as an `error` event, `log_connections` / `log_disconnections` lines as `connection`, `log_lock_waits` reports as `lock_wait`, `log_temp_files` lines as `temp_file` with the spill's exact bytes lifted beside the statement fingerprint (#3602), `log_autovacuum_min_duration` completion reports as `autovacuum` with the run's relation, duration, pages, tuples, buffers and WAL lifted into columns (#3603, V130 — `get_pg_autovacuum_health` shows them per table as `recent_runs`), and checkpoint lines recognised and stored under their family. The same 4 MB tail and the same `logging_collector` gate as `pg_deadlocks`, classified in C# by one `PgLogEventClassifier` both transports feed — the body crosses the wire so no family's recogniser is spelled twice. `message` / `detail` / `context` are redacted with the plan parser's own patterns and the statement is never stored, only fingerprinted; `raw_line_hash` is the identity every read dedupes on. A family whose `log_*` setting is off on the target is empty by construction; the read names the setting |
| `pg_plan_capture_readiness` | `pg_settings` (the `auto_explain` and logging GUCs) | 60 min / 365 d | Whether the target could capture plans at all, one row per facet in causal order — could it run `auto_explain`, is it loaded, is it capturing, in what format, and can a plan be attributed to a statement — because "no plans" has several unrelated causes and each carries its own remedy on the row. Hourly since every facet is a parameter-group setting; a year because "when did plan capture get turned on" is asked months later |
| `pg_write_stats` | `pg_stat_checkpointer`, `pg_stat_bgwriter`, `pg_stat_wal` | 1 min / 30 d | Checkpoints, background writing and WAL as ONE collector because they are one story: a high share of REQUESTED checkpoints means WAL filled `max_wal_size` before the interval elapsed. A union schema with version-conditional columns — PostgreSQL 17 moved the checkpoint counters out of `pg_stat_bgwriter`, so a major that lacks a column reports NULL rather than the collector splitting into per-version twins. PostgreSQL 14+ (`pg_stat_wal`); all three views are cluster-wide singletons, so a snapshot is one row |
| `pg_lock_stats` | `pg_locks` | 1 min / 30 d | Lock state by type, mode and relation, granted or waiting. Does NOT duplicate `pg_blocking_edges`, which stores blocked/blocker PAIRS — that answers who is stuck behind whom, and cannot answer what lock, on what object, is being waited on. Aggregated by (database, locktype, mode, granted, relation), so row count is bounded by CONTENTION rather than concurrency — and it is a SAMPLE, so the cadence is the resolution |
| `pg_replication_stats` | `pg_stat_replication` | 1 min / 30 d | The CONNECTED standbys — four byte distances from `pg_current_wal_lsn()` and three time lags. The counterpart of the slot collector and a different question: a slot describes what the primary RETAINS and exists even when nothing is attached (the disk-filling case); this describes replicas actually attached, and one that disappears here while its slot persists is the dangerous combination. The lag columns keep growing during a stall rather than freezing, which is what makes them usable as an alerting signal |
| `pg_buffer_usage` | `pg_buffercache` | 60 min / 30 d | What is resident in the shared buffer pool, per relation — which objects the cache is SPENT on, a different question from which are read most. Joins `pg_buffercache.relfilenode` to `pg_relation_filenode(c.oid)`, **not** to `c.oid`, which is the join every published example gets wrong, and scopes to the connected database because the pool is cluster-wide while `pg_class` is not. Hourly: the view is a scan of every buffer (6.1 ms at 512 MB, linear to ~780 ms at 64 GB), and a server without the extension records a non-fatal skip naming the one-line `CREATE EXTENSION` fix |
| `pg_extension_availability` | `pg_available_extensions`, `pg_extension` | 24 h / 365 d | Four states, not a boolean — `installed`, `outdated`, `available`, `absent` — because `available` is a `CREATE EXTENSION` away and `absent` is not. Absence is derived against an enumerated roster; `auto_explain` and `pg_wait_sampling` are deliberately off it, being preload-only modules that never appear in `pg_available_extensions` even where they are loaded and working. Per database as of V95, because `installed`/`outdated` are per-database facts |
| `pg_wait_sampling` | the `pg_wait_sampling` extension's profile, or — without the extension — a service-side `pg_stat_activity` sampler (#3604) | 5 min / 30 d | Wait events attributed to the query that waited — wait analysis existed ONLY on Aurora before this, which was backwards, because self-hosted is where the extension story is richest. Two arms, one table, one instrument per target: with the extension created in the monitored database the collector reads its 10 ms in-engine profile; without it the collector polls `pg_stat_activity` once a second for a 30-second window each cycle and accumulates the tallies itself, so a stock first install gets a wait profile (a floor, not parity) rather than an empty chart. Not on Aurora, which has `pg_wait_stats` and cannot load the module. Every read of this table discloses `instrument` (`extension_sampled` / `service_sampled`). `sample_count` is a tally of observations, not a duration, and `profile_period_ms` travels beside it because the count is uninterpretable alone. Since V133 the sampler arm also stores `sampled_ms` — the milliseconds of the interval its 30-second window actually observed — because samples × period over the five-minute interval understates that arm's wait rate by the duty cycle (~10×); the extension arm stores NULL there, having watched the whole interval. Written from that rung, read by nothing yet. Cluster-wide with no database attribution, deliberately: the profile carries none, and inventing one would be the scope error V95 removed elsewhere |
...
| `pg_predicate_stats` | `pg_qualstats` | 60 min / 30 d | Which columns are filtered on, how selectively, and where the planner's estimate was wrong. `pg_index_usage_stats` records indexes that were USED; this records predicates that were EVALUATED, including the ones with no index behind them — the index-candidate signal, invisible to everything else here. `sample_rate` is stored per row and is usually not 1 (the extension defaults to `1/max_connections`), so counts are never read as complete. Per database, because the OIDs its rows carry only resolve in their own database |
| `pg_column_stats` | `pg_stats` | 24 h / 365 d | The planner inputs behind a misestimate — `n_distinct`, `null_frac`, `avg_width`, `correlation`, and the frequency of the single most common value. The SHAPE of the skew, never the values: `most_common_vals` and `histogram_bounds` hold raw customer data and are deliberately not stored. Per database. Daily and kept a year because these move when ANALYZE runs and are asked about retrospectively. Needs `pg_read_all_data` — without it the collector succeeds and stores zero rows, the quiet failure the paragraph above exists for |
| `pg_index_bloat` | `pg_stats`, `pg_class`, `pg_index` | 24 h / 90 d | The complete btree census with no size floor, ESTIMATED from catalog statistics with no page reads since #3234, ranked by reclaimable bytes and never by percentage. A row with a `skipped_reason` has NO answer rather than a healthy one, and every row carries the `pgstatindex` call for its own index as the on-request exact route. **Writers only, and per database.** Daily to match `pg_index_usage_stats` — the two answer halves of one question: is this index earning its keep, and is it wasting space doing it |
| `pg_cpu_utilization` | AWS Performance Insights (`os.cpuUtilization`) | 5 min / 30 d | Instance-level CPU for an Aurora target. This collector has no SQL route at all — PostgreSQL exposes no instance-level CPU signal — so every row arrives through the RDS/Performance Insights API, authorized by the monitoring host's AWS identity rather than any database grant. Five minutes rather than one because each cycle is an outbound `GetResourceMetrics` call per server, and each call asks for the last several 1-minute points, so a missed cycle backfills rather than gaps. Since V136 (#3691) the same row also carries the host's memory — `os.memory.total/free/cached/buffers/active` as bytes and `configured_memory_bytes` (the minute's Serverless v2 capacity × 2 GiB per ACU; NULL on a provisioned class) — from the same call: six more metric names, no second request. A stock PostgreSQL target has no row here at all, and that absence is what a memory consumer must read as *unavailable*, never as "no memory". Read by nothing yet |
| `pg_database_size_stats` | `pg_database`, `pg_database_size()` | 60 min / 365 d | The per-database size SERIES (#3691, V136), the source the object-growth and disk-free questions never had: `size_bytes` per database and `total_bytes` for the instance on every row, templates included and flagged. Hourly because size moves in checkpoint- and autovacuum-sized steps; a year because "how has this grown since last year" is what a size series is for (~6,000 rows a day fleet-wide). A database this role may not size — the vendor-owned one on a managed target — is a row with `size_bytes NULL`, never 0 and never a failed collection, and the instance total goes NULL with it rather than becoming a partial sum. Read by nothing yet |

The two Aurora-only collectors — `pg_wait_stats` and `pg_cpu_utilization`, the Performance Insights CPU read — are gated on Aurora detection, not on configuration: the connect probe looks for `aurora_version()` and the gate follows what it finds. Point Darling at self-managed PostgreSQL and the other 27 collectors run while those two sit out.
...
Three of the 29 are outage predictors rather than performance metrics, which is deliberate: PostgreSQL's most damaging failures are quiet, slow, and fully predictable days ahead, and nothing in the engine raises its hand about them. They are read through the MCP tools (see [the tool list](#mcp)), a web tab and — since #2530 — a WPF viewer tab: a PostgreSQL target gets **seven** inner tabs (Overview, Activity, Vacuum, Waits, I/O, Replication, Storage) in place of the nineteen SQL Server ones, chosen from `collect.servers.engine_kind`. The placement is derived from the collector catalog in both directions, so a new PostgreSQL collector cannot ship without a screen — or without the coverage pin's explicit, dated exemption, which is where `pg_database_size_stats` sits until its consumer lanes land (#3691).

### What it does not do yet

Plan capture shipped (`collect.pg_plan_capture` — `auto_explain` output read from the server log on self-hosted targets and through the AWS log API on Aurora/RDS; #2566, #2538, #2692), closing the largest gap this section used to open with. Scheduled analysis now runs for a PostgreSQL target too: the pass routes by the registry's `engine_kind` inside the analysis service, measures its 24-hour history on `pg_database_stats` and scores the PostgreSQL-target fact vocabulary (`PG_*` / `CONFIG_PG_*` / `ANOMALY_PG_*`, sources `pg_*`) — but the v1 detector families (#3542) are landing lane by lane, so until they do a PostgreSQL pass returns an honest all-clear over the facts it has rather than findings. The PostgreSQL tabs on both UIs are tables over the window: no charts, no correlated timeline, and no drill-down from a blocking root to the sessions behind it. **The three Tier 0 outage predictors DO alert.** Wraparound risk, a blocked vacuum horizon and replication-slot retention are evaluated on the alert cadence and delivered through the same deliverer, history and mute rules as every SQL Server alert, so they land in the same places and obey the same suppression. They ride alongside the shared engine rather than inside it, via a separate `IPostgresAlertReadAdapter` consulted only for PostgreSQL targets — Lite has no PostgreSQL target, and extending the shared adapter would have left it implementing three methods that can only return empty. Their thresholds are derived from the server's own settings (wraparound grades against that cluster's `autovacuum_freeze_max_age`, not a constant) and carry no knob by decision — see [`docs/postgres-alerting-design-note.md`](../docs/postgres-alerting-design-note.md) — but do not read that as "PostgreSQL alerting is not configurable": five more alert families fire on a PostgreSQL target under the SAME names and the same `alerts` switches as SQL Server (`Deadlocks Detected`, `Blocking Detected`, `Long-Running Query`, `Poison Wait`, `High CPU`; #2711, #2719), and the two count alerts have PostgreSQL-specific thresholds — `pgDeadlockCountThreshold` and `pgBlockingCountThreshold` in the table above, `deadlocks.pg_count_threshold` / `blocking.pg_count_threshold` on `get_alert_settings` (#3444, V122). The [first-target runbook](../docs/postgres-first-target-runbook.md#9-alerting)'s step 9 lays all eight out.

Scheduled ANALYSIS runs for a PostgreSQL target through the PostgreSQL-target engine; its finding families are the #3542 v1 build, landing lane by lane.

Collector FAILURES are classified, though. A PostgreSQL fault is routed through the same `ITargetProvider.Classify` the engine seam exposes, so a persistent, operator-actionable condition records as a non-fatal skip with an explanation instead of logging `ERROR` every cycle — a `pg_stat_statements` view that was never created (SQLSTATE 42P01; since #3240 that records as `EXTENSION_MISSING` naming the extension, because the collector declares the dependency), a source Aurora does not implement (0A000), a feature switched off in the parameter group (55006). For the cases that stay in the store's non-fatal `PERMISSIONS` bucket the message says which kind it is, because none of them is a missing grant either. Connection-level failures (the 08 class, 57P0x) still force a reconnect and reprobe; a cancelled statement (57014) deliberately does not, since dropping the connection over a slow query would turn a tuning problem into a reconnect storm. PostgreSQL raises 57014 for the target's own `statement_timeout`, for a `pg_cancel_backend()` and for the client cancel Npgsql sends on its own deadline, so the classification covers all three; which side's deadline fired is read from the server's own message text, and a wording this tool does not recognise is reported as unproven rather than attributed to a machine.
...
| Version | Contents |
|---|---|
| **V1** — collector tables | One table per collector, all 71, generated from the shared collector definitions (column-for-column identical to Lite's DuckDB schema): `wait_stats`, `latch_stats`, `spinlock_stats`, `query_stats`, `procedure_stats`, `query_store_stats`, `query_snapshots`, `plan_cache_stats`, `cpu_utilization_stats`, `cpu_scheduler_stats`, `file_io_stats`, `memory_stats`, `memory_clerks`, `memory_pressure_events`, `tempdb_stats`, `perfmon_stats`, `deadlocks`, `blocked_process_reports`, `dmv_blocking_snapshots`, `memory_grant_stats`, `waiting_tasks`, `session_stats`, `session_summary_stats`, `running_jobs`, `database_size_stats`, `index_object_stats`, `server_properties`, `system_health_events`, the four config snapshots (`server_config`, `database_config`, `database_scoped_config`, `trace_flags`), and the 28 PostgreSQL tables listed under V63–V69, V71, V83–V94, V96–V99, V102, V103, V106 and V129 below |
...
| **V16** — server UTC offset | Nullable UTC-offset column on `server_properties` so the viewer can render timestamps in the monitored server's own local time (the Server-time display mode ported from Lite; Server-time = stored naive-UTC + this offset). It is the offset IN FORCE at collection — `DATEDIFF(MINUTE, GETUTCDATE(), GETDATE())` — so it is exact for an instant on the same side of a DST transition as the collection and an hour wrong for one on the other side, and an offset cannot say which; since **V134** the engine's time-zone id sits beside it for that |
...
| **V94** — PostgreSQL index bloat | `collect.pg_index_bloat` + its time index (#2561): b-tree index bloat, measured via `pgstatindex` **as this rung shipped it**. That decision was correct on its own terms and #3234 reversed it: the estimator route was blind because it needs `pg_stats`, which returns nothing to a `pg_monitor` role, and once `pg_read_all_data` was granted the estimator became viable and the census became the on-request path. Read this row as history — the current behaviour is the grant and extension story above. `avg_leaf_density` is stored RAW and never converted to a percentage, and `skipped_reason` exists so a size ceiling can never masquerade as an absence of bloat |
...
| **V96** — PostgreSQL wait sampling | `collect.pg_wait_sampling` + its time index (#2603): wait events attributed to the query that waited, from the `pg_wait_sampling` extension. Wait analysis existed ONLY on Aurora before this — backwards, because self-hosted is where the extension story is richest. `sample_count` is a tally of observations, not a duration, and `profile_period_ms` travels beside it because the count is uninterpretable alone; storing a derived millisecond figure would bake today's sampling period into history that outlives it. No `database_name`, deliberately: the profile is cluster-wide, and attributing its rows to a database would be exactly the scope error V95 removed from three other tables |
| **V97** — PostgreSQL kernel stats | `collect.pg_kernel_stats` + its time index (#2603): the operating system's own CPU and disk numbers per `queryid`, from `pg_stat_kcache`. `pg_stat_statements` reports elapsed time, which cannot separate a query that was WAITING from one that was burning processor — these compose with it to make that split. `exec_read_bytes` measures I/O that reached the DEVICE (a page-cache hit is genuinely zero), and `stats_since` is stored so a reset is a recorded fact rather than something inferred from a counter that went backwards |
| **V98** — PostgreSQL predicate stats | `collect.pg_predicate_stats` + its time index (#2603): which columns are filtered on, how selectively, and where the planner's estimate was wrong, from `pg_qualstats`. `pg_index_usage_stats` records indexes that were USED; this records predicates that were EVALUATED, including the ones with no index behind them — the index-candidate signal, invisible to everything else here. `sample_rate` is stored per row and is usually not 1 (the extension defaults to `1/max_connections`), so counts can never be read as complete; `worst_estimate_error_ratio` is the reason to look — selectivity says an index might help, the error ratio says the planner does not understand the column |
| **V99** — PostgreSQL plan capture | `collect.pg_plan_capture` + its time index (#2566): `auto_explain`'s JSON plans out of the server log. Its own table, NOT `query_plan_dim` — that one is a documented XML contract with direct-SQL consumers, and JSON rows would break their dashboards with no schema change to notice. No query text and no literals reach the store: identity is `query_id`, every remaining string is redacted before storage, and `plan_hash` is of the REDACTED plan so a shape recurs to one hash whatever values it ran with. Self-hosted reads the log with `pg_read_file`; Aurora and RDS have no filesystem and take the RDS log API instead (#2538) |
| **V102** — PostgreSQL server configuration | `collect.pg_server_config` + its time index (#2658): every `pg_settings` row, hourly for a year — SQL Server answers this three ways and PostgreSQL had no answer at all, and no other part of the stack can reconstruct a configuration history that was never recorded. Nothing is filtered at collection: `source` is stored so the READ can state which rows are server configuration and which are the collector's own session, where the rule can be enforced and explained |
| **V103/V104** — PostgreSQL deadlocks | `collect.pg_deadlocks` + its time index, then the `deadlock_hash` lookup index as its own rung (#2661): the full reports PostgreSQL writes to its log — we collected the COUNT (`pg_stat_database.deadlocks`) and nothing else. `deadlock_hash` is the identity, because the `pg_read_file` route re-reads an OVERLAPPING tail deliberately so one deadlock lands many times, and every read groups on the hash to answer once per deadlock. `graph_text` is stored whole beside the parsed columns, so a lock shape the parser does not recognise still arrives readable by a person. The index is a separate rung because the schema-generator pin permits exactly one index per collector rung — split out, BOTH fresh and upgraded stores get it |
| **V106** — PostgreSQL instance CPU | `collect.pg_cpu_utilization` + its time index (#2719): instance-level CPU for an Aurora target, from the Performance Insights API. This collector has no SQL route at all — PostgreSQL exposes no instance-level CPU signal — so every row arrives through the monitoring host's AWS identity, never a database connection |
| **V129** — PostgreSQL log events | `collect.pg_log_events` + its time index (#3601): the classified server-log events — `occurred_at`, `family`, `severity`, `sqlstate`, `database_name`, `user_name`, `application_name`, `pid`, redacted `message` / `detail` / `context`, `statement_fingerprint` (a hash of the REDACTED statement; the text itself is never stored) and `raw_line_hash`, the identity every read dedupes on because the `pg_read_file` route re-reads an overlapping tail. Column-for-column what the generator emits, like every collector rung, which is why the `(server_id, family, collection_time)` index a family-filtered read over a long window would want is a SEPARATE rung when it comes — V104's reason. Thirty-day retention, from the schedule table. **V130** (#3602, #3603) widened the row rather than adding sibling tables: `relation_name`, `bytes`, `duration_ms`, `pages_removed` / `pages_remaining`, `tuples_removed` / `tuples_remaining`, `buffer_hits` / `buffer_misses` / `buffer_dirtied`, `wal_records` / `wal_bytes`, `is_analyze` — all nullable, no default, no backfill; `bytes` on a `temp_file` event, the rest on an `autovacuum` one, null on every other family. Same ALTER-plus-CREATE pairing as V101: the CREATE carries them for a fresh store, the ALTER for one that climbed through V129 first |
| **V131** — notification routes | `config.config_notification_routes` (#3598): the sparse routes that send alert FAMILIES to different channels than the parent `config_notification` row's — `metric_match` (a family name or an exact metric name), one destination per channel type under the parent's own column names (`teams_url`, `slack_url`, `generic_url`, `pagerduty_routing_key`, `smtp_recipients`; empty = inherit), a GENERATED `configured_channels` presence column so the roles denied the URLs can still say which channels a route sets, `enabled`, `modified_at`, and a V117-shaped `config_version` bump trigger so the running service hot-reloads a route with no restart. Zero rows = the pre-routes fan-out, byte for byte. The four webhook-class destinations join the fail-closed viewer/mcp secret carve; `mcp` may toggle `enabled` and DELETE, never INSERT or write a destination |
| **V133** — PostgreSQL saturation numerator and sampler duty cycle | `collect.pg_database_stats.numbackends` and `collect.pg_wait_sampling.sampled_ms` (#3691), the one rung of the v2 wave: the client backends connected per database at the instant of the read (a level, never differenced — the `max_connections` fraction's honest numerator, which `pg_session_states` could not supply because it captures only sessions holding an old open transaction), and the milliseconds of each five-minute interval the service-side wait sampler actually observed (30,000 on a full window; NULL from the extension arm, which watches the whole interval) so a rate read stops understating that arm ~10×. Both nullable, no default, no backfill, appended last, on compressed hypertables that have no `v_` view to refresh; the V83 / V96 CREATE texts carry them for a fresh store and this rung's two ALTERs for an upgraded one (the V101 rule). Written by the collectors from this rung; both consumers landed later in the same campaign (`numbackends` → the `PG_CONNECTION_SATURATION` fact and `get_pg_database_stats`' `peak_numbackends`; `sampled_ms` → the sampled wait profile's rate). Lite stores no `pg_*` table, so no DuckDB bump |
| **V134** — time honesty | `collect.cpu_utilization_stats.sample_time_utc` and `collect.server_properties.time_zone_id` (#3653 item 13, rulings Q7 + Q8; Lite schema v63 is the twin). `cpu_utilization_stats.sample_time` is the MONITORED SERVER'S LOCAL wall clock — the one column in the store that is not naive UTC, by a documented convention (Lite plots it in the server's frame, and it is the collector's watermark) — so every reader that aligned it against UTC derived the UTC value from an offset (the #1262 per-batch de-skew here, the collected `utc_offset_minutes` on Lite), and both derivations place the samples nearest a DST transition an hour wrong, silently. The collector now writes the same instant in UTC beside the unchanged local stamp (`SYSUTCDATETIME()` through the same arithmetic; on Azure SQL DB `end_time`, which is already UTC), and the readers that window against UTC — the viewer's raw CPU read, the MCP `get_cpu_utilization`, the Lite CPU window — take `COALESCE(sample_time_utc, …)` with the old derivation as the fallback; readers that want the server's frame keep the local column, and the latest-row reads keep it as their within-batch tiebreak (every row has it; an order on the twin would compare a pre-rung local stamp against a post-rung UTC one). The CPU alert gate's latest-row read kept the local stamp as the gate's observation identity at this rung because the gate compared identities by `>` and a one-time frame change would have frozen it for one offset's worth of hours east of UTC; #3744 moved the gate to equality and that read now projects the twin beside the local stamp and hands the gate `sample_time_utc ?? sample_time`. `time_zone_id` is `CURRENT_TIMEZONE_ID()`, the engine's own zone name, beside the offset V16 added — a zone can say which side of a transition an instant fell on where an offset cannot (#3231); NULL where the engine cannot say, which is every SQL Server before 2022 (the function is a missing built-in there, not a missing object, so the collector reads it in its own gated `sp_executesql` batch), and `get_server_properties` publishes the pair and says what a NULL zone means. Both nullable, no default, no backfill — the offset a row's server had at a past sample's instant is exactly what the store never recorded, and no zone can be recovered from an offset; `v_cpu_utilization_stats` is refreshed for the V14 reason, `server_properties` has no view |
| **V135** — Long-Running Query opt-out knob | `config.config_alert_settings.long_running_query_excluded_program_name_prefixes` and `.long_running_query_excluded_logins` (`text[] NOT NULL`, DEFAULT the production read's seeds — the SQL Agent job-step prefix; the two `NT AUTHORITY` service logins) — the #3653 A5/Q5 knob's store home; a pre-rung row reads as the seeds, a cleared list stays `{}` |
| **V136** — PostgreSQL per-database size series and host memory | `collect.pg_database_size_stats` + its time index (#3691), the hourly per-database size series (`size_bytes`, the instance `total_bytes` denormalized per row, `is_template`, `allows_connections`); and six nullable `bigint` columns on `collect.pg_cpu_utilization` — `memory_total_bytes`, `memory_free_bytes`, `memory_cached_bytes`, `memory_buffers_bytes`, `memory_active_bytes`, `configured_memory_bytes` — the host's memory from the same Performance Insights call, on the same row. Columns rather than a `pg_host_memory` table because a 73rd hypertable would have widened the compression band past what the hour can pay (the heaviest refresh's watch line would drop below its 896 s recorded ceiling — a scheduling decision, not this rung's). Nullable, no default, no backfill; V106's CREATE carries the columns for the fresh population (the V101 rule). Nothing reads either yet |
| **V137** — Query Store capture modes, the route knob's store column, the plan dimension's TOAST bytes, and the store's checkpointer phases | Four column sets on three existing tables, one rung (partial #3796, #3712, #3783; Lite schema v64 twins the first). (a) `collect.query_store_health.query_capture_mode` and `.wait_stats_capture_mode` — the `sys.database_query_store_options` `*_desc` spellings verbatim (`ALL` / `AUTO` / `CUSTOM` / `NONE`; `ON` / `OFF`), the one option the health row never asked for and the one that names a plan-churn factory (measured on one production store class: ~755 k new distinct plans a day from 42 servers, with the fleet's other three knobs uniform). `wait_stats_capture_mode_desc` is 2017+, so the shared collector gates that one column and writes NULL on a 2016 engine — a NULL a reader publishes as "engine predates the option", never as `OFF`; `v_query_store_health` is refreshed for the V14 reason. (b) `config.config_alert_settings.analysis_uncorroborated_route` — the store-backed twin of the file-level `analysis.uncorroboratedRoute` knob #3732 shipped, a nullable tri-state (`NULL` = not set in the store, the file or its shipped `digest` governs; `digest` / `page` when set) under a CHECK, because a route has no clamp the way the numeric knobs on this row do and a misspelling would otherwise read as the default rather than as an error. The code half landed in the same release: `DarlingAlertSettings.ResolveUncorroboratedRoute` reads the column over darling.json's `analysis.uncorroboratedRoute` (store non-NULL wins over file, NULL defers, the seed leaves it NULL), `get_alert_settings` publishes the effective route with `analysis.uncorroborated_route_source`, `update_alert_settings` writes `digest` / `page` / null, and the Viewer's Settings → Automated Analysis combo edits it. (c) `collect.store_metrics.toast_bytes` and `.toast_live_bytes`, filled only on `object_kind = 'dimension'` rows: the TOAST relation's file size beside the total the row already stored, so a plan dimension whose TOAST file sits at 40 % utilisation (154 GB holding ~61 GB of live chunks on one production store class, the slack ordinary `VACUUM` never returns to the OS) reads as a finding rather than being discovered on a size pass. `toast_live_bytes` is written NULL, deliberately: every extension-free read was measured on a rig and the tuple-share proxy reads 100 % on exactly that shape, so the column waits for the maintainer's choice of instrument (`pg_freespacemap`'s `file − sum(avail)` measured honest; it needs `CREATE EXTENSION`). (d) `collect.store_metrics.checkpoint_write_ms`, `.checkpoint_sync_ms` and `.checkpoints_requested` — for a `checkpointer` row (`object_kind = 'checkpointer'`, `object_name = 'pg_stat_checkpointer'`) the inventory will write once per run with the DELTAS since its previous run of the store's own checkpointer write-phase ms, sync-phase ms and requested (WAL-forced) checkpoint count, from `pg_stat_checkpointer` on 17+ and `pg_stat_bgwriter` before it; three unattributed read kills on a production store in one day all sat inside 25.2 s and 14.0 s checkpoint sync phases and nothing recorded it (#3783; the levers are #3802's and #3745's). Filled by #3783's code half; NULL until then. All eight nullable, no default, no backfill; no new table or hypertable (the count stays 72). The two capture modes are read since #3796's code half — `get_query_store_health` publishes them as its two trailing fields on both SKUs and both viewers' Query Store grids show them, with a null rendered as an absence, never as `OFF`; the route knob's column is read since #3712's code half (above); nothing reads the other five yet |
...
- **The continuous aggregates compress too** (#3581). The twenty rollups the store materializes — the hourly and daily history tiers, the Query Store dedup layers and the seven anomaly baselines — each get columnar compression on their materialization (segmented by `server_id`, ordered by the bucket column) and a **once-a-day** compression policy of their own. The delay is derived from each tier's refresh window rather than chosen: an hourly-refreshed aggregate compresses chunks older than **2 days** (its refresh re-materializes 1 day back, plus one day of margin), a daily-refreshed one after **4 days** (3 days back, plus the same margin), so a refresh never has to decompress what a policy just compressed. These jobs run **off the hourly phase grid** — one aggregate per hour at `:35` UTC from `01:35` — and on a store that already holds weeks of uncompressed rollups the first runs are **staged one aggregate per night, largest first**, so the backlog pass is one bounded relation a night; a fresh store simply gets all twenty policies at once. Why it matters: an uncompressed rollup of a 9x-compressed raw table is *larger than the raw table* (the hourly grain is only a ~4x row reduction), and on a large store the aggregates were more than half the database. Idempotent under the catalog and re-converged on every start like the raw tier's policies; in `timescaledb_information.jobs` they appear as `policy_compression` rows whose hypertable is `collect.<aggregate view>` with a `1 day` schedule.
- **The materializations are chunked at one day, like the raw tables** (#3620). TimescaleDB's default sizes a rollup's chunks at ten times the raw table's interval, so every aggregate carried 10-day chunks: `drop_chunks` removes only whole chunks, which made the 7-day interval-dedup tier hold anywhere from 7 to 17 days, and a compression policy compresses only whole chunks, which kept the newest ~10 days of every rollup uncompressed whatever its `compress_after`. The same startup ensure now holds every materialization at the raw chunk width (derived from the one constant, so the two tiers cannot drift apart), affecting chunks created from then on; existing wide chunks age out on their own schedule, so a store's short rollups tighten to their designed hold over about two weeks rather than overnight.
...
Today that is **74** and **85** for 72 hypertables (the 71 collector tables plus `collection_log`). The `+ 2` is not slack — it is exactly TimescaleDB's own two built-in jobs, `policy_telemetry` and `policy_job_stat_history_retention`, so a fully migrated store holds precisely one job per worker:
...
| 30 days | Most collector tables (wait/query/procedure/Query Store stats, CPU, memory, file I/O, tempdb, perfmon, deadlocks, blocking, sessions, config snapshots), and `pg_log_events` — an event log sized by the target's `log_*` settings rather than by anything rare, so it gets the common horizon rather than `pg_deadlocks`' 90 (#3601) |
...
| 365 days | `server_properties`, `job_history`, `pg_server_config`, `pg_column_stats`, `pg_extension_availability`, `pg_plan_capture_readiness`, and `pg_database_size_stats` — a size series is asked "since last year" (#3691) |
...
| **Queries** | Sub-tabs over the last 24 hours — **Performance Trends** (a 2×2 of per-second trend charts: query duration, procedure duration, Query Store duration, execution count), **Active Queries** (the ~26-column filterable snapshot grid of captured running queries with a time-range slicer, a **Latest Snapshot** button that re-reads the newest stored capture, and per-row Estimated / Actual plan buttons that open the stored plan in the Plan Viewer), **Top Queries by Duration** (the full query-stats grid with in-grid bar cells for executions/CPU/duration/reads and a CPU-by-database breakdown), **Top Procedures by Duration**, **Query Store by Duration**, **Query Store Regressions**, **Query Store Clutter** (#3797 — which databases make this server's Query Store catalog expensive: per database the collector's read cost, plan churn and options row with the verdict and its reasons, a readable secondary carried as a row with the architectural reason on it rather than filtered out, and the per-server `QDS_*` / memory-clerk overhead proxy in a block of its own; select a row for the recommendation), **Plan Corrections**, and **Query Heatmap** (query counts per 5-minute bin × per-execution magnitude bucket, by a chosen metric; right-click a cell to drill into Active Queries for that window) — the three per-source grids each carry a time-range slicer (drag to narrow the window) and a shared **Compare** control that overlays the current window against a baseline period (yesterday, last week, or same day last week), flagging new and vanished queries |
...
| **Configuration** | Column-filterable snapshot grids of the server's latest capture — server configuration (`sys.configurations`), database configuration (28 columns of `sys.databases`), database-scoped configuration, and trace flags |
...
| **Collection Health** | Sub-tabs — **Health Summary** (a 7-day per-collector roll-up: run / success / error counts, failure rate, average duration, last success / run / error, and a health band of HEALTHY / WARNING / STALE / FAILING / NEVER_RUN / NO_PERMISSIONS / EXTENSION_MISSING — double-click a collector to open its full run history), **Collection Log** (the recent run log with per-run SQL and store-write timings and row counts), and **Duration Trends** (a per-collector success-duration scatter) |
...
Only the visible tab loads (Lite's visible-only rule). The Alerts tab and the visible server tab's active inner tab refresh every 60 seconds; the Overview refreshes on its own faster 30-second timer (Lite's Overview cadence); and **Recommendations** refreshes on tab activation, its Refresh button, and its own server-selector change only, never on the timer — its findings change on the service's 30-minute analysis cadence, so a 60-second auto-refresh would be pointless churn (and would reset the incident expanders under the reader), matching Lite. The one finding that can appear between passes is **same-statement pileup** (evaluated on the collection cadence — see the `alerts` section), so a live pileup shows up here on the next activation or Refresh rather than the instant it fires; its notification does not wait for either.

The viewer has the same three color themes as Lite — Dark, Light and Cool Breeze, picked from **Settings → Color theme** — and the same **Colors** section beneath that drop-down (#3577): one row per adjustable palette color of the theme on screen, with a swatch, the hex, a picker and the measured contrast ratio against the surface it renders on (green ≥ 4.50:1, amber 3.00–4.49, red below — a warning, never a gate). **Apply** writes and applies live, **Reset to default** returns that one theme to its shipped palette, **Open file** hands you the JSON. The file is `%APPDATA%\PerformanceMonitorDarling\theme-overrides.json`, beside `viewer-settings.json`, and it is **local to this machine**: a color is a preference of the person at the screen, so it lives in neither the store nor the control plane, and a second viewer seat keeps its own. The keys, the JSON shape and the parse rules are the same as Lite's — see [Themes and colors in Lite's README](../Lite/README.md#themes-and-colors).
...
- **Delta continuity** — every delta-based collector (wait stats, file I/O, perfmon, memory grants, latch and spinlock stats, procedure stats, query stats, and the PostgreSQL wait and statement stats) re-seeds its baselines from the store at startup — the latest row per key inside a 15-minute window — along with the per-collector pass window the recompiled-plan credit reads, so the first cycle after a restart produces real deltas instead of zeroes. `query_stats` joined the key seed with V128: its delta key is built from the two statement offsets, which the store persists since that rung, so a row written before it (NULL offsets) restores only the pass window and every row written since restores its baseline too.
...
**"TimescaleDB setup failed — continuing in plain-PostgreSQL mode"** (warning) — the extension exists but conversion hit a problem. Everything still works (DELETE-based retention, plain tables), and nothing here waits for a restart any more. The *availability* half: the hourly store-maintenance tick re-probes the extension and logs **"TimescaleDB is available after all"** at Information when it comes back, which restores the store background-job health check (policy-job self-heal, job cadence, Retention Held) that the fallback had switched off (#3815). The *conversion* half: the same tick's store-object convergence pass re-runs every ensure the failed block had reached — so the hypertables, policies and aggregates it did not get to are built on the next pass rather than at the next start (#3817).

**"Store object convergence: 12 steps, 0 changed (none), 0 failed (none), 214 ms"** (information, hourly and once at startup with an `at startup` prefix) — the convergence pass's own receipt (#3817). Every pass writes one, converged or not, for the reason the job-health line does: an absent line used to mean either *every store object is in place* or *the pass never ran and nobody noticed*. **steps** is how many ensures ran; **changed** names the ones that actually altered something (a converged store changes nothing, every hour, and says so); **failed** names any ensure that threw — each of which also logs its own warning, and each of which is retried on the next pass, which is the whole point of the cadence. A store that has just gained a collector or an aggregate shows it here once and then goes quiet. The elapsed is the pass's own clock including connection acquisition: on a converged store it is well under a second, and a pass that takes minutes is either building something for the first time or is on a store too slow to answer its own catalog — the per-object lines above it say which.

**"Store job health: 12 jobs read (compression 6, refresh 3, retention 3), 0 dead, 0 stuck, 1 held, 0 re-armed, 0 alerted"** (information, hourly) — the background-job self-heal's own receipt (#3816). Every pass writes one, healthy or not, because "no alert" used to mean either *nothing is wrong* or *the connection open failed, was swallowed at Debug, and nothing has been checked for days*. What the numbers mean:

- **dead** — `next_start = -infinity` while not running: the scheduler has abandoned the job. Below TimescaleDB 2.26.4 the service re-arms it once (`alter_job(next_start => now())`) and pages; from 2.26.4 on that state is a crash backoff the scheduler clears itself, so the first sighting is logged and left alone and a second consecutive sighting pages without re-arming — re-arming there *resets* the backoff instead of shortening it (#3591, upstream #9360).
- **stuck** — running for longer than its own bound: a run that hung and never finished.
- **held** — `scheduled = false`, which for a retention policy is the [rollup-coverage gate](../docs/retention-hold-runbook.md) working as designed. **Held is counted and never acted on.** A held policy is not a dead one, and arming one by hand drops history no rollup has materialized.
- **re-armed / alerted** — what this pass did about the above.

Each family pages under its own name so a mute or a notification route on one is not silently a mute on the others: `Compression Job Stuck` (critical — the archival tier stops draining and the disk fills), `Refresh Job Stuck` (critical — a rollup stops materializing, every reader of that view gets silently stale answers, and the coverage gate then holds that tier's retention), `Retention Job Stuck` (warning — the store keeps history it was told to drop, which is disk and nothing else). `Store Job Failing` (information) is the fourth: a job whose failure count grew since the previous hourly sample and whose last run reports `Failed` — alive and being retried, but failing, which no other surface reported (#2136's cadence check reads only *successful* runs, so it gets quieter as a job gets sicker).
...
**Recommendations tab says no findings** — analysis runs every 30 minutes per server but only once the store holds at least 24 hours of collected data for that server; a fresh install simply has not earned findings yet. (The exception is **same-statement pileup**, which needs only the current snapshot plus 45 minutes of that statement's own history, so it can appear on a young install while every other finding is still waiting out the data-span gate.)
...
| Store background-job health (policy-job self-heal across compression, rollup refresh and retention; job cadence; Retention Held), the TimescaleDB availability re-probe, the retention coverage re-evaluation and the store-object convergence pass | First sweep after startup, then hourly at :30 past the minute; the availability re-probe runs first and outside the TimescaleDB gate, so a service that fell back to plain-PostgreSQL mode at startup recovers on its own (#3815); a held retention policy arms itself on the first hourly pass after its rollups cover raw, no restart needed (#3812); every pass writes one `Store job health:` line naming what it read, per family (#3816); the store-object convergence pass runs last and re-ensures every store object the start path builds — hypertables, compression policies, continuous aggregates and their refresh windows, the baseline relations, the composer's covering indexes — so one ensure that failed at startup heals within the hour instead of at the next restart, and writes one `Store object convergence:` line naming what it changed and what failed even when that is nothing (#3817) |
...
| `viewer` | SELECT on all of `collect`, and on `config` **minus the secret columns** of `config_monitored_servers` / `config_command` / `config_notification` (carved fail-closed, below) + the web dashboard's narrow single-table writes: INSERT/UPDATE/DELETE on `config.custom_views` (the composer's saved views), on `config.custom_alert_rules` (the user-authored alert rules), on `config.database_state_expected` (the per-database override editor) and on `config.config_mute_rules` (the dedicated mute-rule endpoints, [#3450](https://github.com/erikdarlingdata/PerformanceMonitor/issues/3450)) — plus UPDATE on exactly the two `config_service` reload-beacon columns the mute table's bump trigger writes as the caller. Runs under `statement_timeout = 15s` | a locked-down Viewer (`connectAs: "viewer"`), and the web dashboard |
| `mcp` | `viewer`'s exact read surface + INSERT on `collect.analysis_findings` / `config.analysis_muted` + INSERT/UPDATE/DELETE on `config.custom_views` (the custom-view tools) + the alert-tuning writes (INSERT/UPDATE/DELETE on `config.config_mute_rules`, UPDATE on `config.config_alert_settings`, UPDATE on the single non-secret `email_cooldown_minutes` column of `config.config_notification`, and the `config_service` reload-beacon columns) + the server-onboarding writes (INSERT/UPDATE/DELETE on `config.config_monitored_servers` — the credential column stays SELECT-carved, so it can WRITE a password blob but never READ one back) | the store identity the MCP host connects as whenever `postgres.managed` is `true` — loopback and network alike (`DarlingMcpHostService` builds its pool from the stored `mcp`-role credential in managed mode, never from the owner's); BYO points the MCP host at its own connection string and never creates this role |
...
**What a web token can reach.** The web dashboard connects as the least-privilege `viewer` role and hosts no live-server queries (no `analyze_server`, no plan re-execution) — read-only over the collected data, with a small, enumerated set of **config-plane write routes**, each a narrow single-table grant at the store and each refused to a read-only OIDC seat by the host's method-based write gate: the Custom Views composer (`/api/views` + `/api/compose/run` → `config.custom_views`), the custom alert rules (`/api/alerts` → `config.custom_alert_rules`, [#3285](https://github.com/erikdarlingdata/PerformanceMonitor/issues/3285)), and the mute-rule endpoints (`/api/mute-rules` — create, partial update with explicit-null clears, enable/disable, delete — → `config.config_mute_rules`, [#3450](https://github.com/erikdarlingdata/PerformanceMonitor/issues/3450)); the mute writes run the SAME cores the MCP verbs run, so `created_at_utc` — the clock the Stale Mute Rules self-alert ages a rule from — never moves through any of them, and every response reports the rule as stored. One operational caveat rides every mute-rule surface: alert rows can spell `server_name` two ways (the monitor's own self-alert family uses the display-short name, engine alerts use the registry name), so scope a rule by copying the spelling off the alert rows being muted — a retyped name can leave the rule in force while matching nothing. (The `viewer` role holds one further narrow grant — INSERT/UPDATE/DELETE on `config.database_state_expected`, for the Viewer's per-database state-override editor ([#1986](https://github.com/erikdarlingdata/PerformanceMonitor/issues/1986)) — which no web route reaches; the WPF Viewer's own surfaces gate on a `has_table_privilege` read-only probe against the alert-dismiss write, which a `viewer`-role seat still fails, so a locked-down Viewer stays read-only in its UI despite these grants.) So a token-holder can read everything collected, author saved views and alert rules, and tune the mute rules — and reach no monitored server — but those writes are why an exposed dashboard authenticates loopback too, exactly as MCP does.
