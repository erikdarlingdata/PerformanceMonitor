...
| `uncorroboratedRoute` | `"digest"` | **Where a notify-worthy but UNCORROBORATED finding goes** (#3712). `"digest"` keeps it off email, Teams, Slack, PagerDuty and the generic webhook: it is still persisted, still visible on the web dashboard and MCP the moment it fires, recorded in the alert history as `notification_type: digest` with the gate's reason (`routing_reason` on `get_alert_history`), and named once a day in the **Analysis Singles Digest**. `"page"` restores the pre-#3712 behaviour where every finding at or above `notifySeverity` pages. **File-level** like `web.publicBaseUrl` — not a store column, so a store reload does not change it and `update_alert_settings` refuses it by name; edit the file and restart. |
