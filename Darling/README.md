...
`pg_stat_statements` must be present for `pg_statement_stats`, which means the extension in `shared_preload_libraries` (a restart, or a parameter-group change plus reboot on Aurora/RDS) and `CREATE EXTENSION pg_stat_statements;` in the database Darling connects to. The extension tracks **all** databases in the cluster keyed by `dbid`, so one installation in the connect database covers the whole instance. Four collectors need something installed, and the list is the claim rather than a count: `pg_statement_stats` (`pg_stat_statements`, above), `pg_index_bloat` (`pgstattuple`), `pg_buffer_usage` (`pg_buffercache`), and `pg_wait_sampling` (the `pg_wait_sampling` module). Every other collector needs nothing installed — they read core catalogs and Aurora's built-in functions. All four degrade identically when their extension is absent: the query raises `42P01` or `42883`, which classifies as `ObjectMissing` and is recorded as a non-fatal `PERMISSIONS` skip, and `pg_extension_availability` reports which install would light it up.
