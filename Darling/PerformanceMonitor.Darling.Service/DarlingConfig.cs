...
public sealed class DarlingConfig
{
    ...
    /// <summary>
    /// The auto force-plan bot (#2138 phase 1) — the FIRST feature that can write to a monitored SQL
    /// Server, which is why its defaults are OFF and then DRY RUN. <c>enabled: false</c> (the
    /// default) means the bot evaluates nothing; enabled with <c>dryRun: true</c> (also the default)
    /// journals every would-force/blocked decision to <c>collect.plan_force_actions</c> and touches
    /// no server; a live write additionally requires the per-server
    /// <c>config_monitored_servers.plan_force_bot_enabled</c> opt-in, which nothing in this file can
    /// set. Optional — omit the section entirely and the bot stays off.
    /// </summary>
...
public sealed class ForcePlanBotFileConfig
{
    ...
    /// <summary>First self-review checkpoint after a live force, in minutes.</summary>
