...
public sealed class WebNetworkConfig
{
    ...
    /// <summary>
    /// Optional DNS name the dashboard is also reachable by (#2550). The anti-DNS-rebind Host allowlist
    /// otherwise accepts only <c>localhost</c>, a loopback literal, or the exact listen IP — which is fine for
    /// a bare-IP deployment but refuses (400) every request that arrives under a DNS name, and some identity
    /// providers (Okta among them) accept only DNS-name <c>redirect_uri</c>s. Setting this adds EXACTLY ONE
    /// extra allowed Host; anything else is still rejected as a rebind. The TLS certificate should carry a
    /// dNSName SAN for it (the host skips the iPAddress-SAN warning when it does).
    /// </summary>
