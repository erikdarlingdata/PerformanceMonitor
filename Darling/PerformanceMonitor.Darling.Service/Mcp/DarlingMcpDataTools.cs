...
public sealed class DarlingMcpDataTools
{
    ...
    public static async Task<string> GetCollectionLog(
        NpgsqlDataSource postgres,
        [Description("Server name or display name. Omit or pass \"*\" for the WHOLE FLEET: runs from every enabled server, merged and ranked together, each row carrying server_name (#4199) — this is different from the reserved name (fleet), which reads the fleet-MAINTENANCE run-records instead.")] string? server_name = null,
        ...
        [Description("Limit to runs with this status, matched case-insensitively against the log's own vocabulary: SUCCESS, SKIPPED, YIELDED, ABANDONED, ERROR, PERMISSIONS, EXTENSION_MISSING, SESSION_MISSING, WARNING.")] string? status = null)
    {
        ...
        {
            return await GetCollectionLogFleetAsync(postgres, hours_back, limit, as_of, collector_name, min_duration_ms, status);
        }
