    ...
    internal static string JobHistoryNote(DarlingStoreMetricsReader.JobExecutionLoggingReading reading)
        => reading.Status switch
        {
            ...
            DarlingStoreMetricsReader.JobExecutionLoggingStatus.Off when reading.OffByExplicitOverride =>
                ...
                + "source_file names the file that won). timescaledb_information.job_history is NOT recording, so a "
                + "maximum over it returns zero rows and that means 'this instrument is off', NOT 'no run exceeded the "
                + "line'. The service's managed conf block cannot correct this one: PostgreSQL reads "
                + "postgresql.auto.conf after postgresql.conf, so an ALTER SYSTEM override wins and has to be removed "
                + "(ALTER SYSTEM RESET) before logging can start. Until then the only surface is job_stats — the "
                + "last_run_duration_ms in this response — which carries one sample per job, not one row per run.",
