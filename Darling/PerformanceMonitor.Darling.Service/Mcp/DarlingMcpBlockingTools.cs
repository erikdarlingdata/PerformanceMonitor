...
public sealed class DarlingMcpBlockingTools
{
    ...
    public static async Task<string> GetDeadlocks(
        ...
        [Description("Optional alert fingerprint (the alert's Dedup Key). The fingerprint scan runs over the window BEFORE limit, up to the scan ceiling the payload reports as rows_examined / scan_truncated.")] string? dedup_key = null,
        [Description(McpHelpers.AsOfDescription)] string? as_of = null)
    {
        var (resolved, error) = await DarlingServerResolver.ResolveWithFingerprintNameAsync(postgres, server_name);
        if (error != null) return error;
