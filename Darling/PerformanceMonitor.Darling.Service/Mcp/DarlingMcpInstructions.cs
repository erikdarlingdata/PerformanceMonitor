...
internal static class DarlingMcpInstructions
{
    ...
    private const string Body = """
        ...
        **Tuning the alerting (write).** Three Darling-only tools change the shared alert configuration the service delivers on — the SAME config `get_alert_settings` / `get_mute_rules` read, and the same the Viewer's Settings window writes. They are the only alert writes here; none touches a monitored server or the collected data, and a change hot-reloads into the running service within one collection sweep.
        ...
        | `get_oversized_plan_backlog` | The cached plans measured as too large to capture inline, and what the hourly out-of-band sweep has done about each: per server the three verdict buckets (pending/captured/expired, a strict partition of the total), the attempt figures on still-pending rows, the newest capture and expiry instants, and `observed_bytes` min/median/max, with the per-collector census beside them. `last_captured_at` is how a working fetch half is told from one that has never succeeded. No time window — a worklist updated in place, not a series. | `server_name` (optional; omit for the fleet), `include_rows` (default false; needs `server_name`), `limit` (default 50) |
