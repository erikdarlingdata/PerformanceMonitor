...
public sealed class AnalysisPassTokenThreadingTests
{
    ...
    public void NoStoreCallOnTheAnalysisPassRunsWithoutThePassToken()
    {
        ...
        foreach (var (file, code, source) in AnalysisSources())
        {
            ...
            for (var i = 0; i < code.Length; i++)
            {
                ...
                if (s_exempt.ContainsKey(enclosing) && DocBlockAbove(source, IndexOfDeclaration(code, i)).Contains(ExemptionMarker, StringComparison.Ordinal))
                {
                    continue;
                }
    ...
    /// a doc comment is prefixed on every line by definition, which is what a <c>/* … */</c> body in this
    ...
    /// <see cref="CSharpSourceWalker.StripCommentsAndStrings"/>'s output and is what every regex here reads,
    /// so a commented-out call and a doc comment naming <c>ReadAsync()</c> are neither offenders nor
    /// counted. <c>Source</c> is the file as written, and exists for the two things that are ABOUT the
    /// prose: <see cref="DocBlockAbove"/>, which reads the exemption marker out of a doc comment, and the
    /// offender messages, which should quote what someone will find when they open the file.
    ///
    ...
    private static IEnumerable<(string File, string[] Code, string[] Source)> AnalysisSources()
    {
        ...
        foreach (var file in files.OrderBy(f => f, StringComparer.Ordinal))
        {
            ...
            var source = ReadSource(file).Replace("\r\n", "\n", StringComparison.Ordinal);
