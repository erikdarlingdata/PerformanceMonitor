...
/// must count only <c>SUCCESS</c>/<c>SKIPPED</c> collection_log rows as evidence the instrument is up. An
/// <c>ERROR</c> row after the anchor is proof the collector tried and failed, not proof it is fresh — the
/// bug this fixes was counting ANY status, so a run that failed every time still read "No resolution
/// recorded" instead of the honest "Unknown (not collected since T)".
/// </summary>
