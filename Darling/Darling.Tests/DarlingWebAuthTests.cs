...
public sealed class DarlingWebAuthTests
{
    ...
    /* ---- #2550: the Host allowlist with web.network.publicHost ---- */

    [Theory]
    [InlineData("darling.corp.local", "darling.corp.local", true)]    // the configured name passes
    [InlineData("DARLING.CORP.LOCAL", "darling.corp.local", true)]    // case-insensitive, like localhost
    [InlineData("evil.com", "darling.corp.local", false)]             // one name, not a category
    [InlineData("sub.darling.corp.local", "darling.corp.local", false)] // never a suffix/subdomain match
    [InlineData("darling.corp.local", null, false)]                   // unset -> byte-for-byte the shared guard
    [InlineData("localhost", "darling.corp.local", true)]             // the shared guard's allowances survive
    [InlineData("192.168.1.205", "darling.corp.local", true)]         // …including the listen IP
    public void IsAllowedHost_WithPublicHost(string host, string? publicHost, bool expected)
        => Assert.Equal(expected, DarlingWebHostService.IsAllowedHost(host, IPAddress.Parse("192.168.1.205"), publicHost));

    [Theory]
    [InlineData("darling.corp.local", "darling.corp.local")]
    [InlineData("  Darling.Corp.Local  ", "darling.corp.local")]      // trimmed + lowercased
    [InlineData("10.1.2.3", "10.1.2.3")]                              // an IP literal is a valid extra host (NAT/proxy)
    [InlineData("https://darling.corp.local", null)]                  // a URL is not a host name
    [InlineData("darling.corp.local:5153", null)]                     // no port — the allowlist compares bare hosts
    [InlineData("darling.corp.local/path", null)]
    [InlineData("has space.local", null)]
    [InlineData("", null)]
    [InlineData(null, null)]
    public void NormalizePublicHost_BareHostNamesOnly(string? raw, string? expected)
        => Assert.Equal(expected, DarlingWebHostService.NormalizePublicHost(raw));
