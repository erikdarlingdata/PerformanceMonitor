...
public sealed class TimescaleSupportTests
{
    ...
    /// <summary>
    /// #2316 Finding 2: <c>query_store_stats</c> compresses on its own identity, not on time.
    ///
    /// <para>Measured on TimescaleDB 2.28.1 / PG 18.4 — the deployed pair — over 240,000 rows of the real
    /// 41-bigint shape at 594 bytes/row: <b>2.36x on what shipped, 6.89x with this order-by</b> (57 MB to
    /// 20 MB). The table writes thousands of distinct queries at a single collection time, so under the
    /// default time-descending order the encoder differences one query's metrics against an unrelated
    /// query's. Ordering on the query first makes each query's successive measurements contiguous.</para>
    ///
    /// <para>The control is what makes this a finding rather than a guess: a TIME-first order-by measures
    /// 2.43x — restating the default buys nothing. Pinning the leading column is therefore the whole point,
    /// and a well-meant "add collection_time first, it is the read order" would silently return the table to
    /// 2.4x while still looking like it has an order-by.</para>
    /// </summary>
    [Fact]
    public void CompressionSql_OrdersQueryStoreStatsByQueryIdentity_NotByTime()
    {
        var sql = TimescaleSupport.EnableCompressionSql("query_store_stats");

        Assert.Equal(
            "ALTER TABLE query_store_stats SET (timescaledb.compress, "
            + "timescaledb.compress_segmentby = 'server_id', "
            + "timescaledb.compress_orderby = 'query_id, collection_time DESC')",
            sql);

        /* The identity leads. Anything before it collapses the gain. */
        Assert.StartsWith("query_id", ExtractOrderBy(sql), StringComparison.Ordinal);

        /* Schema-qualified callers resolve the same entry — the runtime path qualifies some tables. */
        Assert.Contains("compress_orderby", TimescaleSupport.EnableCompressionSql("collect.query_store_stats"),
            StringComparison.Ordinal);
    }

    /// <summary>
    /// Every OTHER table is untouched, which is the property that makes this shippable: one measured row
    /// shape got an order-by, and nothing was extrapolated onto shapes that were never measured.
    /// <c>query_stats</c> and <c>procedure_stats</c> are the plausible next candidates — they key on
    /// <c>query_hash</c> and <c>sql_handle</c> respectively, neither has <c>query_id</c>, and both already sit
    /// near 9.5x — so their absence here is deliberate rather than an oversight.
    /// </summary>
    [Fact]
    public void CompressionSql_LeavesEveryOtherTableOnTheDefaultOrder()
    {
        foreach (var schema in CollectorCatalog.All)
        {
            var sql = TimescaleSupport.EnableCompressionSql(schema);
            if (schema.TargetTable.EndsWith("query_store_stats", StringComparison.OrdinalIgnoreCase))
            {
                continue;
            }

            Assert.DoesNotContain("compress_orderby", sql, StringComparison.Ordinal);
        }

        Assert.DoesNotContain("compress_orderby",
            TimescaleSupport.EnableCompressionSql("query_stats"), StringComparison.Ordinal);
        Assert.DoesNotContain("compress_orderby",
            TimescaleSupport.EnableCompressionSql("procedure_stats"), StringComparison.Ordinal);
        Assert.DoesNotContain("compress_orderby",
            TimescaleSupport.EnableCompressionSql(TimescaleSupport.CollectionLogTable), StringComparison.Ordinal);
    }

    private static string ExtractOrderBy(string sql)
    {
        const string marker = "timescaledb.compress_orderby = '";
        var at = sql.IndexOf(marker, StringComparison.Ordinal);
        Assert.True(at >= 0, "no compress_orderby in: " + sql);
        var from = at + marker.Length;

        return sql[from..sql.IndexOf('\'', from)];
    }
