...
/// runner matched by login only, and a real long user query matched by neither.</para>
/// </summary>
public class PgLongRunningQueryExclusionTests
{
    private static LongRunningQueryExclusionSql Build(LongRunningQueryExclusions knob) =>
    ...
    private static string Render(LongRunningQueryExclusions knob, bool excludeBackups = true)
    {
        var sql = Build(knob);
        return DarlingPgSessionStatesReader.BuildCurrentLongRunningSessionsSql(excludeBackups, sql.ProgramPrefixPredicate, sql.LoginPredicate);
    }
    ...
    public void TheRead_FiltersAheadOfTheCap_AndCountsEachArmOnce()
    {
        ...
        var filter = sql.IndexOf("WHERE NOT (s.excluded_by_program_prefix OR s.excluded_by_login)", StringComparison.Ordinal);
    ...
    {
        Assert.ThrowsAny<ArgumentException>(() => DarlingPgSessionStatesReader.BuildCurrentLongRunningSessionsSql(true, "", "FALSE"));
        Assert.ThrowsAny<ArgumentException>(() => DarlingPgSessionStatesReader.BuildCurrentLongRunningSessionsSql(true, "FALSE", " "));
    }
    ...
    public void TheHost_ThreadsTheKnobFromTheSharedSettings_ThroughTheSharedBuilder()
    {
        ...
        Assert.Contains("Details = BuildPgLongRunningQueryExclusionDetails(exclusions, read.ExcludedByProgramPrefix, read.ExcludedByLogin),", evaluator, StringComparison.Ordinal);
    ...
    public void TheReceipt_IsTheSqlServerTwinsItem_AndAbsentForAClearedKnob()
    {
        ...
        var details = DarlingWorker.BuildPgLongRunningQueryExclusionDetails(knob, excludedByProgramPrefix: 2, excludedByLogin: 1);
        ...
        Assert.Empty(DarlingWorker.BuildPgLongRunningQueryExclusionDetails(LongRunningQueryExclusions.None, 0, 0));
    }
...
public sealed class PgLongRunningQueryExclusionLiveTests
{
        ...
        NpgsqlDataSource postgres, DateTime nowUtc, LongRunningQueryExclusions knob, int limit, CancellationToken ct)
    {
        ...
        return DarlingPgSessionStatesReader.GetCurrentLongRunningSessionsAsync(
            ...
            excludeBackups: true, excludedDatabases: null,
            programPrefixPredicate: sql.ProgramPrefixPredicate, loginPredicate: sql.LoginPredicate, exclusionOperands: sql.Operands,
            ct);
    }

    private static async Task PlantAsync(
        NpgsqlConnection connection, long collectionId, DateTime at, long backendId, int pid,
        string applicationName, string username, long durationMs, CancellationToken ct)
    {
...
VALUES ($1, $2, $3, $4, $5, $6, 'appdb', $7, $8, NULL,
