...
/// <summary>
/// #3898 D3 head pins for <c>analyze_query_plan</c> (lane e1, wave E), a twin on Lite
/// (<c>McpPlanTools.cs</c>). The generic
...
/// guardrail facts and that every moved detail is still reachable in the tail.
/// </summary>
public sealed class McpToolGuideHeadsAnalyzeQueryPlanTests
{
    ...
    /// <c>DarlingEngineCapability.NotCollectedStatusAsync</c> first and falls back to
    ...
    /// <summary>D3: nothing in Darling's original prose was dropped, only moved. The one sentence that
    /// differs from Lite's (the source of the plan) opens Darling's own tail, and every other original
    /// sentence -- including the fragments <c>McpDescriptionTruthPinTests</c> /
    /// <c>DarlingMcpPlanToolsTests</c> already pin against the whole compiled description -- is still there
    /// verbatim.</summary>
    [Fact]
    public void Tail_CarriesDarlingsOwnOriginalSentences_Verbatim()
    {
        var tail = McpToolGuideTests.Served(Tool).Tail;
        Assert.NotNull(tail);
        Assert.StartsWith("Analyzes a stored execution plan captured from query stats by query_hash.", tail, StringComparison.Ordinal);
