...
/// <summary>
/// #3898 D3 head pins for <c>get_index_usage</c> (wave E, lane e10). A twin: Lite's copy of this class lives in
/// <c>Lite.Tests/McpToolGuideHeads.IndexUsage.cs</c> and pins the same head text. See
...
public sealed class McpToolGuideHeadsIndexUsageTests
{
    ...
    public void Head_CarriesTheOrderingTrap_TheRestartFloor_AndTheUtcDeSkew_AndStaysAtOrUnder493Served()
    {
        ...
        /* D9: unused/write-only always sort ahead of Active, so a capped, all-Unused answer is not proof the
           server has few Active indexes — the trap #2636 exists to name. */
        Assert.Contains(
    ...
    /// <summary>Nothing the original Darling description said was lost: every original sentence, including the
    /// database_name/limit/matching_index_count/truncated guidance Lite's twin never had, moved to the tail
    /// verbatim (dropcheck pins this at the PR level; this pins the two sentences a reader most needs).</summary>
    [Fact]
    public void Tail_CarriesTheDatabaseFilterGuidance_AndTheClassificationRule()
    {
        ...
        /* D9: database_name matching zero rows while the server has index data elsewhere is status empty, not
           unavailable — a different answer from a truly uncollected server. */
