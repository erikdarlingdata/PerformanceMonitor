...
- **The store names its own slow queries: `get_store_query_stats` ranks the monitoring store's SQL by cost and by the role that ran it** ([#3899]) - Field reports that the web viewer and MCP tools were slow could not be traced to a query from anything the product recorded: the store loaded only TimescaleDB and logged no statement durations. The managed store now preloads `pg_stat_statements` through a new v13 conf block that MERGES it into the effective `shared_preload_libraries` list, so an operator's own libraries survive, and turns `pg_stat_statements.track_utility` off, because provisioning's `ALTER ROLE ... PASSWORD` literals are otherwise recorded verbatim (measured on the bundled PostgreSQL 18.4). A store-object convergence step, run at start and on the hourly tick, creates the extension and two SECURITY DEFINER readers (pinned `search_path`, revoked from PUBLIC, role DDL refused) granted to the admin, viewer and mcp roles, and scrubs any role DDL recorded before utility tracking was off. `get_store_query_stats` (MCP, and the `/api/read` mirror) ranks statements by total, mean or max time, calls, rows or disk reads, attributes each to `viewer` (the web viewer), `mcp`, `admin` (the Darling Viewer) or `owner` (the service) with each role's share of the total, and answers `unavailable` with the remedy when the module is missing, not loaded, or not granted. The viewer and mcp roles also log any statement past 5 s (bind parameters off), which the store-log sweep now retains under `slow_statement`. The Linux compose store gets the same preload on its command line, and `provision-roles.sql` carries the bring-your-own steps. The preload is restart-only, so a managed store picks it up on its next service-owned start. Covered by unit tests for the merge, parse and heal (including the fresh-cluster case that would otherwise have dropped `timescaledb` from the list) and live tests on the bundled runtime proving that a role password never reaches the view, that statements are attributed per role and normalized, and that the scrub removes a password recorded with tracking forced on.
