...
- **The store's own log and statement statistics keep no literal: every retained log entry is masked, and the statement reader shows only normalized DML** ([#3920]) - A second review of [#3915] found text still leaking to the viewer and mcp roles. The store-log sweep kept `statement_timeout`, `lock_timeout`, `deadlock` and `unclassified` entries as the server wrote them, so an ERROR's `STATEMENT:` line carried whatever the failed statement did, a DBA's `ALTER ROLE ... PASSWORD` literal included, for 400 days; and the masking #3915 added for slow statements lost its place on an apostrophe inside a double-quoted identifier or a nested comment, or kept a statement cut inside a literal as it stood. Every retained entry now has its values masked (quoted values, key tuples and a statement's literals; the prefix, identifiers and prose numbers stay), statements through a single-pass SQL lexer that masks every literal spelling and withholds text it cannot read to its end, and rows stored before this build are re-masked in bounded hourly slices while the read masks them on the way out. `get_store_query_stats` shows statement text only for normalized DML (every other statement keeps its timings and reads as withheld), which closes comment-split role DDL and connection strings on a store that tracks utility statements, and its advice about `pg_read_all_stats` now says what that grant opens. The managed store's conf reading de-escapes values the way PostgreSQL does (an include directory ending in a backslash was dropped from the preload merge), reports a preload list PostgreSQL rejects as an Error rather than as fine, and bounds its include walk. Covered by the reviewers' leak cases as regression tests, both statement patterns judged by PostgreSQL's own regex engine, and live tests of the reader and of the re-mask pass on a migrated store.
