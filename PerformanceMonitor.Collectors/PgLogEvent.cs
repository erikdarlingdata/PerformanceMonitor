        ...
        PgLogEventMetrics metrics = default)
    {
        ...
        return new PgLogEvent(
            ...
            /* #3944: the message, and the prose of the detail and context, as PostgreSQL wrote them. #3920: a
               ...
               "UPDATE ... WHERE id = 42"`); that SQL is normalized like a stored statement. The HINT companion is
               deliberately NOT stored: it is advice text, never evidence, and nothing here claims otherwise. */
            Message: PgLogTextRedactor.WithholdPlan(entry.Message) ?? string.Empty,
